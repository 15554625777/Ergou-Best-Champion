...
namespace Support.Plugins
{
    ...
    public class Janna : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddSlider("Combo.R.Health", "使用 R 補血", 15, 1, 100);
        }
        ...
        {
            config.AddBool("Misc.E.Tower", "在塔下 使用 E", true);

            // build aa menu
            var aa = config.AddSubMenu(new Menu("進攻時 使用 E", "Misc.E.AA.Menu"));
            ...
            // build spell menu
            var dmg = config.AddSubMenu(new Menu("使用 E 抵擋法術", "Misc.E.Spell.Menu"));
        ...
        {
            config.AddSlider("Mana.E.Priority.1", "優先使用 E 1", 65, 0, 100);
            config.AddSlider("Mana.E.Priority.2", "優先使用 E  2", 35, 0, 100);
            config.AddSlider("Mana.E.Priority.3", "優先使用 E 3", 10, 0, 100);
        }

        public override void InterruptMenu(Menu config)
        {
            config.AddBool("Gapcloser.Q", "使用 Q 阻止 突進", true);
            config.AddBool("Gapcloser.W", "使用 W 阻止 突進", true);

            config.AddBool("Interrupt.Q", "使用 Q 打斷 法術", true);
            config.AddBool("Interrupt.R", "使用 R 打斷 法術", true);
        }
    }
}