...
namespace Support.Plugins
{
    ...
    public class Blitzcrank : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddSlider("ComboCountR", "對範圍內敵人使用大招", 2, 1, 5);
        }

        public override void MiscMenu(Menu config)
        {
            config.AddBool("Misc.Q.OnAttack", "對目標使用 QA", true);
            config.AddBool("Misc.Q.Block", "用Q 阻止敵方接近", true);
            config.AddSlider("Misc.Q.Block.Distance", "Q限制距離", 400, 0, 800);
        }

        public override void HarassMenu(Menu config)
        {
            config.AddBool("HarassQ", "使用 Q騷擾", true);
        }

        public override void InterruptMenu(Menu config)
        {
            config.AddBool("GapcloserE", "使用 E 阻止 突進", true);
            config.AddBool("GapcloserR", "使用 R 阻止 突進", true);
            config.AddBool("InterruptQ", "使用 Q 打斷 法術", true);
            config.AddBool("InterruptE", "使用 E 打斷 法術", true);
            config.AddBool("InterruptR", "使用 R 打斷 法術", true);
        }
    }
}