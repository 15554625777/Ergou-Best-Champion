...
namespace Support
{
    ...
    public abstract class PluginBase
    {
        ...
        {
            Config = new Menu("辅 助: " + Player.ChampionName, Player.ChampionName, true);
            Config.AddSubMenu(new Menu("走 砍", "Orbwalking"));
            TargetSelector.AddToMenu(Config.AddSubMenu(new Menu("目標 選擇", "Target Selector")));

            ComboConfig = Config.AddSubMenu(new Menu("連 招", "Combo"));
            HarassConfig = Config.AddSubMenu(new Menu("騷 擾", "Harass"));
            ManaConfig = Config.AddSubMenu(new Menu("藍量 限制", "Mana Limiter"));
            ...
            DrawingConfig.AddItem(
                new MenuItem("Target" + ChampionName, "目 标").SetValue(new Circle(true, Color.DodgerBlue)));
            DrawingConfig.AddItem(
