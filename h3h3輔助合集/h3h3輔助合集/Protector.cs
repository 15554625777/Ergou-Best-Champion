...
namespace Support
{
    ...
    internal class Protector
    {
        ...
        private static void CreateMenu()
        {
            ...
            // detector targeted
            var targeted = detector.AddSubMenu(new Menu("目 標", "Targeted"));
            targeted.AddItem(new MenuItem("TargetedActive", "啟 用").SetValue(true));
            ...
            // detector skillshots
            var skillshot = detector.AddSubMenu(new Menu("技能 指標", "Skillshots"));
            skillshot.AddItem(new MenuItem("SkillshotsActive", "啟 用").SetValue(true));
            skillshot.AddItem(new MenuItem("IsAboutToHitTime", "時間 延遲").SetValue(new Slider(200, 0, 400)));
            foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(h => h.IsAlly))
            {
                skillshot.AddItem(
            ...
            // spells
            var spells = Menu.AddSubMenu(new Menu("法 術", "Spells"));
            ...
            // items
            var items = Menu.AddSubMenu(new Menu("項 目", "Items"));
            ...
            // misc
            var misc = Menu.AddSubMenu(new Menu("雜 項", "Misc"));
            misc.AddItem(new MenuItem("UsePackets", "使用 封包").SetValue(true));
