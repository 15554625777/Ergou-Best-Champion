    ...
    class Velkoz : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var key = new Menu("键位", "Key");
            {
                ...
                key.AddItem(new MenuItem("R_Mouse", "R 鼠标方向").SetValue(new KeyBind("R".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }

            //Spell Menu
            var spell = new Menu("法术菜单", "Spell");
            {
                //Q Menu
                var qMenu = new Menu("Q 法术", "QSpell"); {
                    qMenu.AddItem(new MenuItem("qSplit", "自动 Q").SetValue(true));
                    qMenu.AddItem(new MenuItem("qAngle", "智能 Q 角度").SetValue(true));
                    spell.AddSubMenu(qMenu);
                }   
                //R
                var rMenu = new Menu("R 法术", "RSpell");
                {
                    rMenu.AddItem(new MenuItem("rAimer", "R 目标").SetValue(new StringList(new[] { "自动", "鼠标位置" })));
                    rMenu.AddSubMenu(new Menu("禁用 R", "DontUlt"));
            ...
            var combo = new Menu("连招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("qHit", "Q 命中率").SetValue(new Slider(3, 1, 4)));
            ...
            var harass = new Menu("骚扰", "Harass");
            {
                ...
                harass.AddItem(new MenuItem("qHit2", "Q 命中率").SetValue(new Slider(3, 1, 4)));
            ...
            {
                misc.AddItem(new MenuItem("UseInt", "使用 E 中断技能").SetValue(true));
                misc.AddItem(new MenuItem("UseGap", "使用 E 防止突进").SetValue(true));
                misc.AddItem(new MenuItem("smartKS", "使用 智能击杀").SetValue(true));
                misc.AddItem(new MenuItem("mana", "点燃前检测魔量是否足够连招").SetValue(true));
            ...
            var draw = new Menu("范围", "Drawings"); { 
                ...
                draw.AddItem(new MenuItem("drawUlt", "显示 R 击杀提示").SetValue(true));
