    ...
    class Yasuo : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var spellMenu = new Menu("法术菜单", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Auto", "自动 Q 目标")).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle));
                    qMenu.AddItem(new MenuItem("Q_Auto_third", "自动 Q3 击飞")).SetValue(true);
                    qMenu.AddItem(new MenuItem("Q_UnderTower", "塔下 自动 Q")).SetValue(false);
                    qMenu.AddItem(new MenuItem("Q_Stack", "手动 Q3 目标")).SetValue(new KeyBind("I".ToCharArray()[0], KeyBindType.Toggle));
                    qMenu.AddItem(new MenuItem("Q_thirdE", "优先 E->3rd Q -> 单一 Q")).SetValue(true);
                ...
                {
                    //wind wall
                    var dangerous = new Menu("躲避 危险", "Dodge Dangerous");
                    {
                ...
                {
                    eMenu.AddItem(new MenuItem("E_Min_Dist", "使用 E 最小距离").SetValue(new Slider(250, 1, 475)));
                    //e Evade
                    var dangerous = new Menu("躲避 危险", "Dodge Spells");
                    {
                        foreach (var hero in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy))
                        {
                            dangerous.AddSubMenu(new Menu(hero.ChampionName, hero.ChampionName));
                    ...
                    eMenu.AddItem(new MenuItem("E_GapClose", "使用 E 突进").SetValue(true));
                ...
                var rMenu = new Menu("R 法术", "RMenu");
                {
                    ...
                    rMenu.AddItem(new MenuItem("R_MEC", "自动R>= 敌人数量, 0 = 关闭")).SetValue(new Slider(3, 0, 5));
            ...
            var combo = new Menu("连招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("qHit", "Q3 命中率").SetValue(new Slider(2, 1, 3)));
            ...
            var harass = new Menu("骚扰", "Harass");
            {
                ...
                harass.AddItem(new MenuItem("qHit2", "Q3 命中率").SetValue(new Slider(2, 1, 3)));
                harass.AddItem(new MenuItem("UseEHarass", "使用 E").SetValue(true));
                //add to menu
                menu.AddSubMenu(harass);
            }
            var farm = new Menu("清线", "Farming");
            {
                farm.AddItem(new MenuItem("UseQLast", "使用 Q 补刀").SetValue(true));
                farm.AddItem(new MenuItem("UseELast", "使用 E 补刀").SetValue(true));
                farm.AddItem(new MenuItem("UseQFarm", "使用Q 清线").SetValue(true));
                farm.AddItem(new MenuItem("UseQ3Farm", "使用 Q3 清线").SetValue(true));
                farm.AddItem(new MenuItem("UseEFarm", "使用 E 清线").SetValue(true));
                farm.AddItem(new MenuItem("E_UnderTower_Farm", "塔下使用E清线")).SetValue(false);
                farm.AddItem(new MenuItem("LaneClear_useQ_minHit", "使用Q小兵数量").SetValue(new Slider(2, 1, 6)));
                //add to menu
            ...
            {
                misc.AddItem(new MenuItem("smartKS", "使用智能击杀系统").SetValue(true));
            ...
            var drawMenu = new Menu("范围", "Drawing");
            {
                ...
                drawMenu.AddItem(new MenuItem("Draw_Q2", "范围 Q 延伸").SetValue(true));
