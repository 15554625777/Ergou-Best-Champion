...
namespace Xerath
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Create the menu
            Config = new Menu("Esk0r 泽拉斯", "Xerath", true);
            ...
            Config.SubMenu("R").AddItem(new MenuItem("EnableRUsage", "自动 使用 R").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("rMode", "R模式").SetValue(new StringList(new[] { "正常", "自定义延迟", "优先"})));
            Config.SubMenu("R").AddItem(new MenuItem("rModeKey", "优先R 键位").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            ...
            Config.SubMenu("R").AddItem(new MenuItem("PingRKillable", "击杀提示 (仅本地)").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("BlockMovement", "右鍵停止R（再次点击继续）").SetValue(false));
            Config.SubMenu("R").AddItem(new MenuItem("OnlyNearMouse", "R向鼠标附近敌人").SetValue(false));
            Config.SubMenu("R").AddItem(new MenuItem("MRadius", "R半径范围").SetValue(new Slider(700, 1500, 300)));

            //Harass menu:
            Config.AddSubMenu(new Menu("骚扰", "Harass"));
            Config.SubMenu("Harass").AddItem(new MenuItem("UseQHarass", "使用 Q 骚扰").SetValue(true));
            Config.SubMenu("Harass").AddItem(new MenuItem("UseWHarass", "使用 W 骚扰").SetValue(false));
            ...
            Config.SubMenu("JungleFarm").AddItem(new MenuItem("UseQJFarm", "使用 Q 清野").SetValue(true));
            Config.SubMenu("JungleFarm").AddItem(new MenuItem("UseWJFarm", "使用 W 清野" ).SetValue(true));
            ...
            Config.SubMenu("Misc").AddItem(new MenuItem("InterruptSpells", "自动E 中断法术").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("AutoEGC", "自动E 防止突进").SetValue(true));


            //Damage after combo:
            var dmgAfterComboItem = new MenuItem("DamageAfterR", "显示3次R的总伤害").SetValue(true);
            ...
            Config.SubMenu("Drawings")
                .AddItem(
                    new MenuItem("QRange", "显示 Q 范围").SetValue(new Circle(true, Color.FromArgb(150, Color.DodgerBlue))));
            Config.SubMenu("Drawings")
                .AddItem(
                    new MenuItem("WRange", "显示 W 范围").SetValue(new Circle(true, Color.FromArgb(150, Color.DodgerBlue))));
            Config.SubMenu("Drawings")
                .AddItem(
                    new MenuItem("ERange", "显示 E 范围").SetValue(new Circle(false, Color.FromArgb(150, Color.DodgerBlue))));
            Config.SubMenu("Drawings")
                .AddItem(
                    new MenuItem("RRange", "显示 R 范围").SetValue(new Circle(false, Color.FromArgb(150, Color.DodgerBlue))));
            Config.SubMenu("Drawings")
                .AddItem(
                    new MenuItem("RRangeM", "显示 R 范围 (小地图)").SetValue(new Circle(false,
            ...
            Game.PrintChat("Esk0r |娉芥媺鏂瘄：鍔犺級鎴愬姛!婕㈠寲by浜岀嫍!QQ缇361630847!");
