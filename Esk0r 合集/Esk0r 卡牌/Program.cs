...
namespace TwistedFate
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            /* Q */
            var q = new Menu("Q - 万能牌", "Q");
            q.AddItem(new MenuItem("AutoQI", "自动-Q 不动的").SetValue(true));
            q.AddItem(new MenuItem("AutoQD", "自动-Q 移动的").SetValue(true));
            q.AddItem(new MenuItem("CastQ", "使用 Q (半自动)").SetValue(new KeyBind("U".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(q);

            /* W */
            var w = new Menu("W - 选牌", "W");
            w.AddItem(
                new MenuItem("SelectYellow", "选择 黄牌").SetValue(new KeyBind("W".ToCharArray()[0],
                    KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectBlue", "选择 蓝牌").SetValue(new KeyBind("E".ToCharArray()[0], KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectRed", "选择 红牌").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(w);

            var r = new Menu("R - 命运", "R");
            r.AddItem(new MenuItem("AutoY", "R后自动选择黄牌").SetValue(true));
            Config.AddSubMenu(r);

            var misc = new Menu("杂项", "Misc");
            misc.AddItem(new MenuItem("PingLH", "残血敌人自动R (仅在范围内)").SetValue(true));
            ...
            /*Drawing*/
            var Drawings = new Menu("范围选项", "Drawings");
            ...
            Drawings.AddItem(new MenuItem("Rcircle2", "R 范围 (小地图)").SetValue(new Circle(true, Color.FromArgb(255, 255, 255, 255))));
