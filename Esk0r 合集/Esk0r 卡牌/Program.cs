...
namespace TwistedFate
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Make the menu
            Config = new Menu("卡牌大師   By:Feeeez", "TwistedFate", true);

            var TargetSelectorMenu = new Menu("選擇目標", "Target Selector");
            ...
            /* Q */
            var q = new Menu("Q - 通配符", "Q");
            q.AddItem(new MenuItem("AutoQI", "自動-Q 靜止的").SetValue(true));
            q.AddItem(new MenuItem("AutoQD", "自動-Q 活躍的").SetValue(true));
            q.AddItem(new MenuItem("CastQ", "釋放 Q (有藍就放)").SetValue(new KeyBind("U".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(q);

            /* W */
            var w = new Menu("W - 选一张牌", "W");
            w.AddItem(
                new MenuItem("SelectYellow", "黃牌").SetValue(new KeyBind("W".ToCharArray()[0],
                    KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectBlue", "藍牌").SetValue(new KeyBind("E".ToCharArray()[0], KeyBindType.Press)));
            w.AddItem(
                new MenuItem("SelectRed", "紅牌").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            Config.AddSubMenu(w);

            var r = new Menu("R - 命運之兆", "R");
            r.AddItem(new MenuItem("AutoY", "选择黄卡后 R").SetValue(true));
            Config.AddSubMenu(r);

            var misc = new Menu("額外選項", "Misc");
            misc.AddItem(new MenuItem("PingLH", "Ping 低血量敵人(本地顯示)").SetValue(true));
            Config.AddSubMenu(misc);

            //Damage after combo:
            var dmgAfterComboItem = new MenuItem("DamageAfterCombo", "連招後顯示傷害").SetValue(true);
            ...
            /*Drawing*/
            var Drawings = new Menu("顯示範圍", "Drawings");
            Drawings.AddItem(new MenuItem("Qcircle", "Q 範圍").SetValue(new Circle(true, Color.FromArgb(100,255,0,255))));
            Drawings.AddItem(new MenuItem("Rcircle", "R 範圍").SetValue(new Circle(true, Color.FromArgb(100, 255, 255, 255))));
            Drawings.AddItem(new MenuItem("Rcircle2", "R 範圍 (小地圖)").SetValue(new Circle(true, Color.FromArgb(255, 255, 255, 255))));
            Drawings.AddItem(dmgAfterComboItem);
            Config.AddSubMenu(Drawings);

            Config.AddItem(new MenuItem("連招", "Combo").SetValue(new KeyBind(32, KeyBindType.Press)));
