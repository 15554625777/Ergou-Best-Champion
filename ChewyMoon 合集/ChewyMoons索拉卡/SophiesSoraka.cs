...
namespace Sophies_Soraka
{
    internal class SophiesSoraka
    {
        ...
        {
            Menu = new Menu("Sophies's Soraka [By: Feeeez]", "sSoraka", true);

            // Target Selector
            var tsMenu = new Menu("目標選擇", "ssTS");
            ...
            // Drawing
            var drawingMenu = new Menu("顯示範圍", "ssDrawing");
            ...
            // Misc
            var miscMenu = new Menu("額外選項", "ssMisc");
            miscMenu.AddItem(new MenuItem("packets", "使用 封包").SetValue(true));
            miscMenu.AddItem(new MenuItem("useQGapcloser", "突進者使用Q中斷").SetValue(true));
            miscMenu.AddItem(new MenuItem("useEGapcloser", "突進者使用E中斷").SetValue(true));
            miscMenu.AddItem(new MenuItem("autoW", "自動使用 W").SetValue(true));
            miscMenu.AddItem(new MenuItem("autoWPercent", "% Percent").SetValue(new Slider(50)));
            miscMenu.AddItem(new MenuItem("autoR", "自動使用 R").SetValue(true));
            miscMenu.AddItem(new MenuItem("autoRPercent", "% Percent").SetValue(new Slider(15)));
            miscMenu.AddItem(new MenuItem("eInterrupt", "使用E打斷目標釋放危險技能").SetValue(true));
