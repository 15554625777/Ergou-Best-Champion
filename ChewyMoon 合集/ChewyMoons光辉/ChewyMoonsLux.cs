...
namespace ChewyMoonsLux
{
    internal class ChewyMoonsLux
    {
        ...
        private static void OnDraw(EventArgs args)
        {
			...
			Game.PrintChat("<font color=\"#6699ff\"><b>ChewyMoon's 鍏夎緣:</b></font> <font color=\"#FFFFFF\">" +
        ...
        private static void SetupMenu()
        {
            ...
            // Target Selector
            var tsMenu = new Menu("[Chewy's 光辉] - 目标选择", "cmLuxTs");
            TargetSelector.AddToMenu(tsMenu);
            Menu.AddSubMenu(tsMenu);

            // Orbwalker
            var orbwalkerMenu = new Menu("[Chewy's 光辉] - 走砍", "cmLuxOrbwalker");
            Orbwalker = new Orbwalking.Orbwalker(orbwalkerMenu);
            Menu.AddSubMenu(orbwalkerMenu);

            // Combo settings
            var comboMenu = new Menu("[Chewy's 光辉] - 连招", "cmLuxCombo");
            ...
            // Harass Settings
            var harassMenu = new Menu("[Chewy's 光辉] - 骚扰", "cmLuxHarass");
            ...
            var ksMenu = new Menu("[Chewy's 光辉] - 抢人头", "cmLuxKS");
            ksMenu.AddItem(new MenuItem("ultKS", "使用 R 抢人头").SetValue(true));
            //ksMenu.AddItem(new MenuItem("recallExploitKS", "KS enemies recalling").SetValue(true));
            Menu.AddSubMenu(ksMenu);

            // Items
            var itemsMenu = new Menu("[Chewy's 光辉] - 物品", "cmLuxItems");
            itemsMenu.AddItem(new MenuItem("useDFG", "使用 冥火之拥").SetValue(true));
            Menu.AddSubMenu(itemsMenu);

            //Drawing
            var drawingMenu = new Menu("[Chewy's 光辉] - 范围", "cmLuxDrawing");
            drawingMenu.AddItem(new MenuItem("drawQ", "显示 Q 范围").SetValue(true));
            drawingMenu.AddItem(new MenuItem("drawW", "显示 W 范围").SetValue(false));
            ...
            // Misc
            var miscMenu = new Menu("[Chewy's 光辉] - 杂项", "cmLuxMisc");
            miscMenu.AddItem(new MenuItem("antiGapCloserQ", "防止 突进").SetValue(true));
            miscMenu.AddItem(new MenuItem("packetCast", "技能 封包").SetValue(false));
            miscMenu.AddItem(
