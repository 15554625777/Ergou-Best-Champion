...
namespace YasuoSharpV2
{
        internal class YasuoSharp
        {
            ...
            private static void onLoad(EventArgs args)
            {
                ...
                Game.PrintChat("浜氱储SharpV2 by DeTuKs 鍔犺浇鎴愬姛!姹夊寲by浜岀嫍!QQ缇361630847");

                try
                {

                    Config = new Menu("亚索Sharp v2", "Yasuo", true);
                    //Orbwalker
                    ...
                    Config.SubMenu("combo").AddItem(new MenuItem("useRHit", "使用R|击飞人数")).SetValue(new Slider(3, 5, 1));
                    Config.SubMenu("combo").AddItem(new MenuItem("useRHitTime", "使用R|敌人击飞")).SetValue(true);
                    Config.SubMenu("combo").AddItem(new MenuItem("useEWall", "使用 E 保命")).SetValue(true);
                    //Flee away
                    Config.SubMenu("combo").AddItem(new MenuItem("flee", "E 逃跑")).SetValue(new KeyBind('Z', KeyBindType.Press, false));
                    ...
                    Config.SubMenu("lClear").AddItem(new MenuItem("useEmpQHit", "使用Q|小兵数量")).SetValue(new Slider(3, 6, 1));
                    Config.SubMenu("lClear").AddItem(new MenuItem("useElc", "使用 E")).SetValue(true);
                    //Harass
                    Config.AddSubMenu(new Menu("骚扰 Sharp", "harass"));
                    Config.SubMenu("harass").AddItem(new MenuItem("harassTower", "塔下禁用骚扰")).SetValue(false);
                    Config.SubMenu("harass").AddItem(new MenuItem("harassOn", "使用骚扰|众多敌人")).SetValue(true);
                    Config.SubMenu("harass").AddItem(new MenuItem("harQ3Only", "仅使用 Q3")).SetValue(false);
                    //Drawings
                    Config.AddSubMenu(new Menu("范围 Sharp", "drawing"));
                    Config.SubMenu("drawing").AddItem(new MenuItem("disDraw", "禁用 所有范围")).SetValue(false);
                    Config.SubMenu("drawing").AddItem(new MenuItem("drawQ", "显示 Q 范围")).SetValue(true);
                    Config.SubMenu("drawing").AddItem(new MenuItem("drawE", "显示 E 范围")).SetValue(true);
                    Config.SubMenu("drawing").AddItem(new MenuItem("drawR", "显示 R 范围")).SetValue(true);
                    Config.SubMenu("drawing").AddItem(new MenuItem("drawWJ", "显示 穿墙 位置")).SetValue(true);

                    //Extra
                    Config.AddSubMenu(new Menu("额外 Sharp", "extra"));
                    Config.SubMenu("extra").AddItem(new MenuItem("djTur", "塔下禁用E")).SetValue(true);
                    Config.SubMenu("extra").AddItem(new MenuItem("autoLevel", "自动加点")).SetValue(true);
                    Config.SubMenu("extra").AddItem(new MenuItem("levUpSeq", "")).SetValue(new StringList(new string[2] { "Q E W Q 加点顺序", "Q E Q W 加点顺序" }));

                    //LastHit
                    Config.AddSubMenu(new Menu("阻挡 技能", "aShots"));
                    //SmartW
                    Config.SubMenu("aShots").AddItem(new MenuItem("smartW", "智能 W")).SetValue(true);
                    Config.SubMenu("aShots").AddItem(new MenuItem("smartEDogue", "使用 E 躲避技能")).SetValue(true);
                    Config.SubMenu("aShots").AddItem(new MenuItem("wwDanger", "W只阻挡危险技能")).SetValue(false);
                    Config.SubMenu("aShots").AddItem(new MenuItem("wwDmg", "W阻挡|法术伤害量")).SetValue(new Slider(0, 100, 1));
                    skillShotMenu = getSkilshotMenu();
                    Config.SubMenu("aShots").AddSubMenu(skillShotMenu);
                    //Debug
                    Config.AddSubMenu(new Menu("调试", "debug"));
                    Config.SubMenu("debug").AddItem(new MenuItem("WWLast", "手动 W 抵挡")).SetValue(new KeyBind('T', KeyBindType.Press, false));
                    Config.SubMenu("debug").AddItem(new MenuItem("saveDash", "保留 位移")).SetValue(new KeyBind('O', KeyBindType.Press, false));
                    Config.SubMenu("debug").AddItem(new MenuItem("exportDash", "使用 位移")).SetValue(new KeyBind('P', KeyBindType.Press, false));
                    Config.SubMenu("debug").AddItem(new MenuItem("deleteDash", "禁用 位移")).SetValue(new KeyBind('I', KeyBindType.Press, false));
