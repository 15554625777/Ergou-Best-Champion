...
namespace RivenSharp
{
    class RivenSharp
    {
        ...
        private static void onLoad(EventArgs args)
        {
            ...
            Game.PrintChat("鐟炴枃Sharp v2 by DeTuKs 鍔犺級鎴愬姛锛佹饥鍖朾y浜岀嫍锛丵Q缇361630847");
            Config = new Menu("瑞文 - Sharp", "Riven", true);
           ...
           Config.AddSubMenu(TargetSelectorMenu);
            ...
            Config.SubMenu("combo").AddItem(new MenuItem("forceQE", " 先|E 后|Q")).SetValue(true);
            Config.SubMenu("combo").AddItem(new MenuItem("packets", "使用 封包")).SetValue(true);

            //Haras
            Config.AddSubMenu(new Menu("骚扰 Sharp", "haras"));
            Config.SubMenu("haras").AddItem(new MenuItem("doHarasE", "使用 E 骚扰")).SetValue(new KeyBind('G', KeyBindType.Press, false));
            Config.SubMenu("haras").AddItem(new MenuItem("doHarasQ", "使用 Q 骚扰")).SetValue(new KeyBind('T', KeyBindType.Press, false));

            //Drawing
            Config.AddSubMenu(new Menu("范围 Sharp", "draw"));
            Config.SubMenu("draw").AddItem(new MenuItem("doDraw", "禁用 范围")).SetValue(false);
            Config.SubMenu("draw").AddItem(new MenuItem("drawHp", "显示 伤害 血量")).SetValue(true);

            //Debug
            Config.AddSubMenu(new Menu("调试 Sharp", "debug"));
            Config.SubMenu("debug").AddItem(new MenuItem("db_targ", "调试 目标")).SetValue(new KeyBind('0', KeyBindType.Press, false));
