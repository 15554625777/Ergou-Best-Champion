...
namespace Sion
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Make the menu
            Config = new Menu("iMeh 塞恩", "Sion", true);
            ...
            Config.AddSubMenu(new Menu("R 设置", "R"));
            Config.SubMenu("R").AddItem(new MenuItem("AntiCamLock", "避免锁定视角").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("MoveToMouse", "R跟随鼠标 (利用BUG)").SetValue(false));
                //Disabled by default since its not legit Keepo


            Config.AddToMainMenu();

            Game.PrintChat("iMeh 濉炴仼 鍔犺級鎴愬姛锛佹饥鍖朾y浜岀嫍锛丵Q缇361630847!!!");
