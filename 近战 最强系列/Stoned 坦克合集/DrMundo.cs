...
namespace StonedSeriesAIO
{
    internal class DrMundo
    {
        ...
        private void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config = new Menu("Stoned 蒙多", "StonedMundo", true);
            ...
            Config.SubMenu("Harass").AddItem(new MenuItem("CountHarass", "禁用骚扰|血量低于").SetValue(new Slider(30, 100, 0)));
            Config.SubMenu("Harass").AddItem(new MenuItem("Harass Tog", "使用 骚扰 (自动)").SetValue<KeyBind>(new KeyBind('T', KeyBindType.Toggle)));
            Config.SubMenu("Harass").AddItem(new MenuItem("ActiveHarass", "骚扰!").SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
            ...
            Config.SubMenu("Wave").AddItem(new MenuItem("ActiveWave", "惊险 键位").SetValue(new KeyBind("V".ToCharArray()[0], KeyBindType.Press)));

            Config.AddSubMenu(new Menu("杂项", "Misc"));
            Config.SubMenu("Misc").AddItem(new MenuItem("KS", "使用 Q 抢人头").SetValue(false));
            Config.SubMenu("Misc").AddItem(new MenuItem("Rsave", "使用 R 恢复").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("Rhp", "开启大招|血量低于").SetValue(new Slider(30, 100, 0)));
