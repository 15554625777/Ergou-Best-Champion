...
namespace UniversalMinimapHack
{
    internal class Program
    {
        ...
        private void Game_OnGameLoad(EventArgs args)
        {
            try
            {
                Menu menu = new Menu("小地图消失计时", "UniversalMinimapHack", true);
                _slider = new MenuItem("scale", "头像比例 % (F5 重载)").SetValue(new Slider(20));
                IconOpacity = new MenuItem("opacity", "头像透明度 % (F5 重载)").SetValue(new Slider(70));
                SsTimerEnabler =
                    new MenuItem("enableSS", "启用").SetValue(true);
                SsTimerSize = new MenuItem("sizeSS", "SS 文本 大小 ((F5 重载)").SetValue(new Slider(15));
                SsTimerOffset = new MenuItem("offsetSS", "SS 高度大小").SetValue(new Slider(15, -50, +50));
                SsTimerMin = new MenuItem("minSS", "显示延迟 X 秒").SetValue(new Slider(30, 1, 180));
                SsTimerMinPing = new MenuItem("minPingSS", "提示延迟 X 秒").SetValue(new Slider(30, 5, 180));
                _ssFallbackPing = new MenuItem("fallbackSS", "回程 提示 (本地)").SetValue(false);
                menu.AddItem(new MenuItem("", "[自定义]"));
                menu.AddItem(_slider);
                menu.AddItem(IconOpacity);
                Menu ssMenu = new Menu("消失计时", "ssTimer");
                ...
                Menu ssCircleMenu = new Menu("消失范围圈", "ccCircles");
                _ssCircle = new MenuItem("ssCircle", "启用").SetValue(true);
                SsCircleSize = new MenuItem("ssCircleSize", "极限范围圈大小").SetValue(new Slider(7000, 500, 15000));
                _ssCircleColor = new MenuItem("ssCircleColor", "范围圈 颜色").SetValue(System.Drawing.Color.Green);
        ...
        {
            Game.PrintChat(
                "<font color='#ff3232'>Universal</font><font color='#d4d4d4'>灏忓湴鍥炬秷澶辫鏃秥鍔犺級鎴愬姛锛佹饥鍖朾y浜岀嫍  浜岀嫍QQ缇361630847</font> <font color='#FFFFFF'>" +
                msg + "</font>");
        }
