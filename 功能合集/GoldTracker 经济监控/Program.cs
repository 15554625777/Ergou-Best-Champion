...
namespace GoldTracker
{
    class Program
    {
        ...
        {
            var moveSpeed = ObjectManager.Player.MoveSpeed;
            var mana = ObjectManager.Player.MaxMana;
            var healthRegeneration = ObjectManager.Player.PARRegenRate;
            Game.PrintChat("ms: {0}, mana: {1}, hr: {2}", moveSpeed, mana, healthRegeneration);
        ...
        {
            menu = new Menu("经济监控", "goldTracker", true);
            menu.AddItem(new MenuItem("displayHeroValues", "显示英雄经济").SetValue(true));
            menu.AddItem(new MenuItem("displayTeamValues", "显示团队经济").SetValue(true));
            menu.AddItem(new MenuItem("includeNonCombatStats", "包括非戰鬥统计 (移动速度, 法力值, ...)").SetValue(true));
            menu.AddToMainMenu();
