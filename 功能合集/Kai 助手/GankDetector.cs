...
namespace KaiHelper
{
    ...
    public class GankDetector
    {
        ...
        {
            MenuGank = config.AddSubMenu(new Menu("Gank 警告", "GDetect"));
            MenuGank.AddItem(new MenuItem("InvisibleTime", "消失 时间").SetValue(new Slider(5, 1, 10)));
            MenuGank.AddItem(new MenuItem("VisibleTime", "可见 时间").SetValue(new Slider(3, 1, 5)));
            MenuGank.AddItem(new MenuItem("TriggerRange", "触发 范围").SetValue(new Slider(3000, 1, 3000)));
            MenuGank.AddItem(new MenuItem("CircalRange", "追踪 范围").SetValue(new Slider(2500, 1, 3000)));
            //MenuGank.AddItem(new MenuItem("Ping", "Ping").SetValue(new StringList(new[] {"Local Ping", "Server Ping"})));
            MenuGank.AddItem(new MenuItem("Fill", "Fill（填充）").SetValue(true));
            ...
            try
            {
                ...
                foreach (var enemy in _enemies)
                {
                    ...
                    if (!_enemies[hero].Pinged)
                    {
                        ...
                        Game.PrintChat("<font color = \"#FF0000\">璀﹀憡锛侊紒锛乬ank: </font>" + hero.ChampionName);
