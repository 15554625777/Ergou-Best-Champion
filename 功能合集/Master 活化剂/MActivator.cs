...
namespace MasterActivator
{
    internal class MActivator
    {
        ...
        MItem muramana = new MItem("Muramana", "魔宗", "muramana", 3042, ItemTypeId.Buff);
        MItem seraph = new MItem("Seraph's Embrace", "炽天使", "seraph", 3040, ItemTypeId.Deffensive);
        ...
        private void createMenuItem(MItem item, int defaultValue, String parent, bool mana = false, bool useOn = true)
        {
            ...
            if (useOn)
            {
                ...
                Config.SubMenu(parent).AddItem(new MenuItem(item.menuVariable + "UseOnPercent", "使用 " + (mana == false ? "最低HP" : "最低MP"))).SetValue(new Slider(defaultValue, 0, 100));
            }
        }
        ...
        private void createMenuSpell(MItem item, int defaultValue, String parent, int minManaPct, bool useMana, bool useOn = true)
        {
            ...
            if (abilitySlot != SpellSlot.Unknown && abilitySlot == item.abilitySlot)
            {
                ...
                if (useOn)
                {
                    Config.SubMenu(parent).AddItem(new MenuItem(item.menuVariable + "MinHpPct", "最低伤害")).SetValue(new Slider(10, 0, 100));
                    Config.SubMenu(parent).AddItem(new MenuItem(item.menuVariable + "UseOnPercent", "最低HP")).SetValue(new Slider(defaultValue, 0, 100));
                    if (useMana)
                    {
                        Config.SubMenu(parent).AddItem(new MenuItem(item.menuVariable + "UseManaPct", "最低MP")).SetValue(new Slider(minManaPct, 0, 100));
                    }
        ...
        private void createMenu()
        {
            ...
            Config.SubMenu("purifiers").AddItem(new MenuItem("defJustOnCombo", "在连招中使用")).SetValue(true);
            ...
            else
            {
                ...
                Config.SubMenu("smiteCfg").AddItem(new MenuItem("LizardElder", "红buff")).SetValue(true);
                ...
                Config.SubMenu("smiteCfg").AddItem(new MenuItem("Wraith", "小幽灵")).SetValue(false);
            ...
            Config.SubMenu("smiteCfg").AddItem(new MenuItem("dSmite", "惩戒范围")).SetValue(true);
            Config.SubMenu("smiteCfg").AddItem(new MenuItem("justAS", "始终开启")).SetValue(false);

            Config.AddSubMenu(new Menu("攻击物品", "offensive"));
            createMenuItem(ignite, 100, "offensive", false, false);
            Config.SubMenu("offensive").AddItem(new MenuItem("overIgnite", "在点燃后使用")).SetValue(false);
            ...
            Config.AddSubMenu(new Menu("加血|加蓝", "regenerators"));
            ...
            Config.SubMenu("combo").AddItem(new MenuItem("comboModeActive", "启用活化剂")).SetValue(new KeyBind(32, KeyBindType.Press, true));

            // Target selector
            Config.AddSubMenu(new Menu("目标选择", "targetSelector"));
            Config.SubMenu("targetSelector").AddItem(new MenuItem("targetMode", "")).SetValue(new StringList(new[] { "低血量", "高AD输出", "高AP输出", "附近队友", "接近鼠标", "自动净化", "低输出", "辅助" }, 0));

            Config.AddItem(new MenuItem("predict", "预测 DMG")).SetValue(true);
            Config.AddItem(new MenuItem("enabled", "功能开启or关闭")).SetValue(true);
