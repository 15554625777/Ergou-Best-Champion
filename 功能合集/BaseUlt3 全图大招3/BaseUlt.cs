...
namespace BaseUlt3
{
    ...
    internal class BaseUlt
    {
        ...
        public BaseUlt()
        {
            (Menu = new Menu("全图大招3", "BaseUlt3", true)).AddToMainMenu();
            Menu.AddItem(new MenuItem("showRecalls", "显示 回程").SetValue(true));
            Menu.AddItem(new MenuItem("baseUlt", "启用 全图大招").SetValue(true));
            Menu.AddItem(new MenuItem("checkCollision", "检测 抵触").SetValue(true));
