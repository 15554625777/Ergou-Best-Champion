    ...
    internal static class Program
    {
        ...
        private static void OnGameLoad(EventArgs args)
        {
            ...
            foreach (KeyValuePair<SpellSlot, int> entry in SpellShots)
            {
                ...
                subMenu.AddItem(MakeSlider(entry.Key.ToString() + "extra", "等级内启用加点(首尾)", 1, 1, 18));
                _menu.AddSubMenu(subMenu);
            }

            _activate = new MenuItem("activate", "启用等级").SetValue(new StringList(new[] { "2", "3" }));
        ...
        {
            Game.PrintChat(
                "<font color='#ff3232'>Universal</font><font color='#d4d4d4'>鑷姩鍔犵偣:鍔犺級鎴愬姛锛佹饥鍖朾y浜岀嫍  浜岀嫍QQ缇361630847</font> <font color='#FFFFFF'>" +
                msg + "</font>");
        }
