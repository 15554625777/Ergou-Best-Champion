...
namespace VeigarLittleEvil
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            menu.SubMenu("Keys")
                .AddItem(
                    new MenuItem("LastHitQQ", "使用 Q 补刀").SetValue(new KeyBind("A".ToCharArray()[0],
                        KeyBindType.Press)));
            menu.SubMenu("Keys")
                .AddItem(
                    new MenuItem("LastHitQQ2", "Q 补刀(保持)").SetValue(new KeyBind("J".ToCharArray()[0],
                        KeyBindType.Toggle)));
            menu.SubMenu("Keys")
                .AddItem(
                    new MenuItem("wPoke", "只对眩晕目标W").SetValue(new KeyBind("N".ToCharArray()[0],
                        KeyBindType.Toggle)));
            menu.SubMenu("Keys")
                .AddItem(
                    new MenuItem("escape", "逃跑").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
