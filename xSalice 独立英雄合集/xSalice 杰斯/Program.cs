...
namespace JayceTheTwerker
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Create the menu
            menu = new Menu("未来守护者-杰斯", "Lissandra", true);
            ...
            menu.SubMenu("Keys").AddItem(new MenuItem("shootMouse", "EQ鼠标方向").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));

            //Combo menu:
            menu.AddSubMenu(new Menu("连招", "Combo"));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseQCombo", "使用 炮形态 Q").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("qSpeed", "EQ 速度").SetValue(new Slider(1600, 400, 2500)));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseWCombo", "使用 炮形态 W").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseECombo", "使用 炮形态 E").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseQComboHam", "使用 锤形态 Q").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseWComboHam", "使用 锤形态 W").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseEComboHam", "使用 锤形态 E").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseRCombo", "使用 R 转换").SetValue(true));
            
            //Harass menu:
            menu.AddSubMenu(new Menu("骚扰", "Harass"));
            menu.SubMenu("Harass").AddItem(new MenuItem("UseQHarass", "使用 Q").SetValue(true));
            menu.SubMenu("Harass").AddItem(new MenuItem("UseWHarass", "使用 W").SetValue(true));
            menu.SubMenu("Harass").AddItem(new MenuItem("UseEHarass", "使用 E").SetValue(true));
            menu.SubMenu("Harass").AddItem(new MenuItem("UseQHarassHam", "使用 Q 锤形态").SetValue(true));
            menu.SubMenu("Harass").AddItem(new MenuItem("UseWHarassHam", "使用 W 锤形态").SetValue(true));
            menu.SubMenu("Harass").AddItem(new MenuItem("UseEHarassHam", "使用 E 锤形态").SetValue(true));
            menu.SubMenu("Harass").AddItem(new MenuItem("UseRHarass", "使用 R 转换").SetValue(true));
            menu.SubMenu("Harass").AddItem(new MenuItem("manaH", "骚扰最低蓝量").SetValue(new Slider(40, 0, 100)));

            //Misc Menu:
            menu.AddSubMenu(new Menu("杂项", "Misc"));
            menu.SubMenu("Misc").AddItem(new MenuItem("UseInt", "使用 E 中断法术").SetValue(true));
            ...
            menu.SubMenu("Misc").AddItem(new MenuItem("forceGate", "先Q 后E").SetValue(false));
            menu.SubMenu("Misc").AddItem(new MenuItem("gatePlace", "门的距离").SetValue(new Slider(300, 250, 600)));
            menu.SubMenu("Misc").AddItem(new MenuItem("UseQAlways", "使用 Q 当 E 未冷却").SetValue(true));
            menu.SubMenu("Misc").AddItem(new MenuItem("autoE", "E连招使用|HP < %").SetValue(new Slider(40, 0, 100)));
            menu.SubMenu("Misc").AddItem(new MenuItem("smartKS", "智能 抢人头").SetValue(true));
            menu.SubMenu("Misc").AddItem(new MenuItem("lagMode", "Bad PC 模式(EQ二连)").SetValue(false));
            ...
            menu.SubMenu("Drawings")
                .AddItem(new MenuItem("drawcds", "显示 技能冷却")).SetValue(true);
            menu.SubMenu("Drawings")
            ...
            Game.PrintChat("|鏈潵瀹堟姢鑰厊-鏉版柉--- by xSalice 鍔犺級鎴愬姛锛佹饥鍖朾y浜岀嫍锛丵Q缇361630847");
        }
