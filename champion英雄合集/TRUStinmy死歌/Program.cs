...
namespace TRUStinmyKarthus
{
    public class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            {
                //Create the menu
                Config = new Menu("TRUStInMy死歌", "mainmenu", true);

                Config.AddSubMenu(new Menu("键位 设置:", "hotkeys"));
                Config.SubMenu("hotkeys").AddItem(new MenuItem("ComboKey", "连招!").SetValue(new KeyBind("V".ToCharArray()[0], KeyBindType.Press)));
                Config.SubMenu("hotkeys").AddItem(new MenuItem("HarassKey", "骚扰").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
                Config.SubMenu("hotkeys").AddItem(new MenuItem("FarmKeyFreeze", "使用 Q 补刀").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Press)));
                Config.SubMenu("hotkeys").AddItem(new MenuItem("LaneClear", "使用 Q 清线").SetValue(new KeyBind("H".ToCharArray()[0], KeyBindType.Press)));

                Config.AddSubMenu(new Menu("R 击杀提示:", "ultlogic"));
                Config.SubMenu("ultlogic").AddItem(new MenuItem("useRauto", "R 击杀提示").SetValue(true));

                Config.AddSubMenu(new Menu("连招 设置:", "combospells"));
                Config.SubMenu("combospells").AddItem(new MenuItem("UseI", "使用 点燃 击杀").SetValue(true));
                Config.SubMenu("combospells").AddItem(new MenuItem("dfg", "使用 冥火").SetValue(true));
                Config.SubMenu("combospells").AddItem(new MenuItem("useQ", "使用 - Q").SetValue(true));
                Config.SubMenu("combospells").AddItem(new MenuItem("useW", "使用 - W").SetValue(true));
                Config.SubMenu("combospells").AddItem(new MenuItem("useE", "使用 - E").SetValue(true));

                Config.AddSubMenu(new Menu("骚扰 设置:", "harassspells"));
                Config.SubMenu("harassspells").AddItem(new MenuItem("useQHarass", "使用 - Q").SetValue(true));
                Config.SubMenu("harassspells").AddItem(new MenuItem("useEHarass", "使用 - E").SetValue(true));

                Config.AddSubMenu(new Menu("显示 设置:", "drawing"));
                Config.SubMenu("drawing").AddItem(new MenuItem("noDraw", "禁用 - 显示").SetValue(true));
                //Config.SubMenu("drawing").AddItem(new MenuItem("drawDmg", "Draw - Damage Marks").SetValue(true));
                Config.SubMenu("drawing").AddItem(new MenuItem("drawQ", "显示 - Q").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 255, 0, 255))));
                Config.SubMenu("drawing").AddItem(new MenuItem("drawW", "显示 - W").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 255, 0, 255))));
                Config.SubMenu("drawing").AddItem(new MenuItem("drawE", "显示 - E").SetValue(new Circle(true, System.Drawing.Color.FromArgb(100, 255, 0, 255))));
