...
public class yetAnotherDiana
{
    ...
    public static void Game_onGameLoad(EventArgs args)
    {
        ...
        {
            Game.PrintChat("婕㈠寲by 鑺辫竟锛丵Q缇361630847!");
            return;
        }
        ...
        //Main Menu
        Config = new Menu("yA-皎月", "yA-Diana", true);
        ...
        //Target Selector
        var targetSelector = new Menu("目标 选择", "Target Selector");
        TargetSelector.AddToMenu(targetSelector);
        Config.AddSubMenu(targetSelector);

        //Combo
        Config.AddSubMenu(new Menu("连 招", "Combo"));
        Config.SubMenu("Combo").AddItem(new MenuItem("Combo-Key", "按 键").SetValue(new KeyBind(32, KeyBindType.Press))); //Spacebar
        Config.SubMenu("Combo").AddItem(new MenuItem("Combo-Use-W", "使用 W").SetValue(true));
        Config.SubMenu("Combo").AddItem(new MenuItem("Combo-Use-E", "使用 E").SetValue(true));
        Config.SubMenu("Combo").AddItem(new MenuItem("Combo-Jump-To-Target", "跳到目标上").SetValue(new StringList(new[] {
			"Killable",
			"On",
			"Off"
		})));
        ...
        Config.SubMenu("Killsteal").AddItem(new MenuItem("Killsteal-Enabled", "开启").SetValue(true));
        Config.SubMenu("Killsteal").AddItem(new MenuItem("Killsteal-Use-Q", "使用 Q").SetValue(true));
        Config.SubMenu("Killsteal").AddItem(new MenuItem("Killsteal-Use-R", "使用 R").SetValue(true));
        Config.SubMenu("Killsteal").AddItem(new MenuItem("Killsteal-Use-Ignite", "使用 点燃").SetValue(true));

        //Harass
        Config.AddSubMenu(new Menu("骚 扰", "Harass"));
        Config.SubMenu("Harass").AddItem(new MenuItem("Harass-Key", "按 键").SetValue(new KeyBind(67, KeyBindType.Press))); //C
        Config.SubMenu("Harass").AddItem(new MenuItem("Harass-Use-Q", "使用 Q").SetValue(true));
        Config.SubMenu("Harass").AddItem(new MenuItem("Harass-Use-W", "使用 W").SetValue(true));
        Config.SubMenu("Harass").AddItem(new MenuItem("Harass-MoveTo", "移动到 鼠标所点").SetValue(true));

        //Farm
        Config.AddSubMenu(new Menu("打 钱", "Farm"));
        Config.SubMenu("Farm").AddItem(new MenuItem("Farm-Key", "开启").SetValue(new KeyBind(86, KeyBindType.Press))); // V
        Config.SubMenu("Farm").AddItem(new MenuItem("Use-Q-Farm", "使用 Q").SetValue(true));
        Config.SubMenu("Farm").AddItem(new MenuItem("Use-W-Farm", "使用 W").SetValue(true));
        Config.SubMenu("Farm").AddItem(new MenuItem("Farm-Mana", "蓝量 限制").SetValue(new Slider(20)));

        //Jungle Farm
        Config.AddSubMenu(new Menu("清 野", "Jungle Farm"));
        Config.SubMenu("Jungle Farm").AddItem(new MenuItem("Jungle-Farm-Key", "按 键").SetValue(new KeyBind(86, KeyBindType.Press))); //V
        Config.SubMenu("Jungle Farm").AddItem(new MenuItem("Use-Q-Jungle", "使用 Q").SetValue(true));
        Config.SubMenu("Jungle Farm").AddItem(new MenuItem("Use-W-Jungle", "使用 W").SetValue(true));
        Config.SubMenu("Jungle Farm").AddItem(new MenuItem("Use-R-Jungle", "使用 R").SetValue(new StringList(new[] {
			"With Q",
			"On",
			"Off"
		})));
        Config.SubMenu("Jungle Farm").AddItem(new MenuItem("Jungle-Mana", "蓝量 限制").SetValue(new Slider(20)));

        //Items	
        Config.AddSubMenu(new Menu("项 目", "Items"));
        Config.SubMenu("Items").AddItem(new MenuItem("Items-Enabled", "开 启").SetValue(true));
        //'Offensive
        Config.SubMenu("Items").AddSubMenu(new Menu("Offense", "进 攻"));
        Config.SubMenu("Items").SubMenu("Offense").AddItem(new MenuItem("BilgeCut", "水银 弯刀").SetValue(true));
        Config.SubMenu("Items").SubMenu("Offense").AddItem(new MenuItem("BotRK", "破败王者之刃").SetValue(true));
        Config.SubMenu("Items").SubMenu("Offense").AddItem(new MenuItem("DFG", "冥火之拥").SetValue(true));
        Config.SubMenu("Items").SubMenu("Offense").AddItem(new MenuItem("RavHydra", "九头蛇").SetValue(true));
        Config.SubMenu("Items").SubMenu("Offense").AddItem(new MenuItem("RanOmen", "兰顿之兆").SetValue(true));
        Config.SubMenu("Items").SubMenu("Offense").AddItem(new MenuItem("Tiamat", "提亚玛特").SetValue(true));

        //'Defensive	
        Config.SubMenu("Items").AddSubMenu(new Menu("Defense", "防 守"));
        //>LoTIS
        Config.SubMenu("Items").SubMenu("Defense").AddSubMenu(new Menu("LoT Iron Solari", "LoTIS-菜单"));
        Config.SubMenu("Items").SubMenu("Defense").SubMenu("LoTIS-Menu").AddItem(new MenuItem("LoTIS", "开启").SetValue(true));
        Config.SubMenu("Items").SubMenu("Defense").SubMenu("LoTIS-Menu").AddItem(new MenuItem("LoTIS-HP-%", "HP %").SetValue(new Slider(40)));
        //>Zhonya's
        Config.SubMenu("Items").SubMenu("Defense").AddSubMenu(new Menu("Zhonya's Hourglass", "中亚-菜单"));
        Config.SubMenu("Items").SubMenu("Defense").SubMenu("Zhonya-Menu").AddItem(new MenuItem("Zhonya", "开启").SetValue(true));
        ...
        Config.SubMenu("Misc").AddItem(new MenuItem("Auto Level", "自动 加点").SetValue(true));
        Config.SubMenu("Misc").AddItem(new MenuItem("Packet Casting", "使用 封包").SetValue(true));
        Config.SubMenu("Misc").AddItem(new MenuItem("Quick Start", "快速 启动").SetValue(new StringList(new[] {
			"Off",
			"CF,HP,2*MP",
			"CF,3*HP",
			"Jungle: HM,4*HP"
        ...
        Config.SubMenu("Drawing").AddItem(new MenuItem("Draw", "范 围").SetValue(true));
        Config.SubMenu("Drawing").AddItem(new MenuItem("Draw R", "范 围 R").SetValue(new Circle(true, System.Drawing.Color.DeepPink)));
        //>Enemy Status
        foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.IsEnemy))
        {
            ...
            Config.SubMenu("Drawing").SubMenu(enemy.ChampionName).AddItem(new MenuItem(enemy.ChampionName + "E", "开启").SetValue(true));
            Config.SubMenu("Drawing").SubMenu(enemy.ChampionName).AddItem(new MenuItem(enemy.ChampionName + "KC", "击杀 线圈").SetValue(true));
            ...
            Config.SubMenu("Drawing").SubMenu(enemy.ChampionName).AddItem(new MenuItem(enemy.ChampionName + "R", "范 围").SetValue(new StringList(new[] {
				"Basic",
				"Q",
				"W",
				"E",
            ...
            Config.SubMenu("Drawing").SubMenu(enemy.ChampionName).AddItem(new MenuItem(enemy.ChampionName + "RC", "范围 颜色").SetValue(new Circle(true, System.Drawing.Color.Gray)));
        }

        //Interrupt
        Config.AddSubMenu(new Menu("打 断", "Interrupt"));
        Config.SubMenu("Interrupt").AddItem(new MenuItem("Interrupt-Enabled", "开启").SetValue(new StringList(new[] {
			"Off in Combo",
			"On",
			"Off"
		},1)));
        Config.SubMenu("Interrupt").AddItem(new MenuItem("Interrupt-Use-E", "使用 E").SetValue(true));
        Config.SubMenu("Interrupt").AddItem(new MenuItem("Interrupt-Use-R", "使用 R").SetValue(new StringList(new[] {
			"Danger",
			"On",
			"Off"
		})));
        ...
        if (Utility.Map.GetMap()._MapType == Utility.Map.MapType.SummonersRift)
        {
            ...
            //>Jungle Jump
            Config.SubMenu("Jungle Farm").AddSubMenu(new Menu("野区 跳跃", "Jungle Jump"));
            Config.SubMenu("Jungle Farm").SubMenu("Jungle Jump").AddItem(new MenuItem("Jungle-Jump-Key", "按 键").SetValue(new KeyBind(71, KeyBindType.Press))); //G
            Config.SubMenu("Jungle Farm").SubMenu("Jungle Jump").AddItem(new MenuItem("Jungle-Draw-Spots", "显示 位置").SetValue(new Circle(true, System.Drawing.Color.Blue)));
            Config.SubMenu("Jungle Farm").SubMenu("Jungle Jump").AddItem(new MenuItem("Jungle-MoveTo", "移动到 鼠标之处").SetValue(true));
        }
        else
            Dfg = new Items.Item(3188, 750);

        //Gap Closer
        Config.AddSubMenu(new Menu("阻止 突进", "Gap Closer"));
        Config.SubMenu("Gap Closer").AddItem(new MenuItem("GapCloser-Use-W", "使用 W").SetValue(true));
