...
namespace Master_Yi_by_Prunes
{
    public class LxOrbwalker
    {
        ...
        public static void AddToMenu(Menu menu)
        {
            ...
            var menuDrawing = new Menu("范 围", "lxOrbwalker_Draw");
            menuDrawing.AddItem(new MenuItem("lxOrbwalker_Draw_AARange", "攻击 范围").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(new MenuItem("lxOrbwalker_Draw_AARange_Enemy", "敌人在 攻击范围内").SetValue(new Circle(true, Color.Pink)));
            menuDrawing.AddItem(new MenuItem("lxOrbwalker_Draw_Holdzone", "Holdzone").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(new MenuItem("lxOrbwalker_Draw_MinionHPBar", "小兵 血量挑").SetValue(new Circle(true, Color.Black)));
            menuDrawing.AddItem(new MenuItem("lxOrbwalker_Draw_MinionHPBar_thickness", "血量条 厚度").SetValue(new Slider(1, 1, 3)));
            menuDrawing.AddItem(new MenuItem("lxOrbwalker_Draw_hitbox", "显示 伤害").SetValue(new Circle(true, Color.FloralWhite)));
            menuDrawing.AddItem(new MenuItem("lxOrbwalker_Draw_Lasthit", "小兵 补刀血量").SetValue(new Circle(true, Color.Lime)));
            menuDrawing.AddItem(new MenuItem("lxOrbwalker_Draw_nearKill", "靠近的小兵 补刀").SetValue(new Circle(true, Color.Gold)));
            menu.AddSubMenu(menuDrawing);

            var menuMisc = new Menu("杂 项", "lxOrbwalker_Misc");
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_Holdzone", "防 守").SetValue(new Slider(50, 100, 0)));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_Farmdelay", "打钱 延迟").SetValue(new Slider(0, 200, 0)));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_ExtraWindUp", "额外 骚扰时间").SetValue(new Slider(80, 200, 0)));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_AutoWindUp", "自动设置 骚扰").SetValue(false));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_Priority_Unit", "优先 Unit").SetValue(new StringList(new[] { "Minion", "Hero" })));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_Humanizer", "人性化 延迟").SetValue(new Slider(50, 100, 0)));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_AllMovementDisabled", "禁用 所有 移动").SetValue(false));
            menuMisc.AddItem(new MenuItem("lxOrbwalker_Misc_AllAttackDisabled", "禁用 所有 攻击").SetValue(false));

            menu.AddSubMenu(menuMisc);

            var menuMelee = new Menu("近 身", "lxOrbwalker_Melee");
            menuMelee.AddItem(new MenuItem("lxOrbwalker_Melee_Prediction", "移动 预测").SetValue(false));
            menu.AddSubMenu(menuMelee);

            var menuModes = new Menu("走砍 模式", "lxOrbwalker_Modes");
            {
                var modeCombo = new Menu("连 招", "lxOrbwalker_Modes_Combo");
                modeCombo.AddItem(new MenuItem("Combo_Key", "按 键").SetValue(new KeyBind(32, KeyBindType.Press)));
                modeCombo.AddItem(new MenuItem("Combo_move", "移 动").SetValue(true));
                modeCombo.AddItem(new MenuItem("Combo_attack", "攻 击").SetValue(true));
                menuModes.AddSubMenu(modeCombo);

                var modeHarass = new Menu("骚 扰", "lxOrbwalker_Modes_Harass");
                modeHarass.AddItem(new MenuItem("Harass_Key", "按 键").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
                modeHarass.AddItem(new MenuItem("Harass_move", "移 动").SetValue(true));
                modeHarass.AddItem(new MenuItem("Harass_attack", "攻 击").SetValue(true));
                modeHarass.AddItem(new MenuItem("Harass_Lasthit", "补 刀").SetValue(true));
                menuModes.AddSubMenu(modeHarass);

                var modeLaneClear = new Menu("清 线", "lxOrbwalker_Modes_LaneClear");
                modeLaneClear.AddItem(new MenuItem("LaneClear_Key", "按 键").SetValue(new KeyBind("V".ToCharArray()[0], KeyBindType.Press)));
                modeLaneClear.AddItem(new MenuItem("LaneClear_move", "移 动").SetValue(true));
                modeLaneClear.AddItem(new MenuItem("LaneClear_attack", "攻 击").SetValue(true));
                menuModes.AddSubMenu(modeLaneClear);

                var modeLaneFreeze = new Menu("控 线", "lxOrbwalker_Modes_LaneFreeze");
                modeLaneFreeze.AddItem(new MenuItem("LaneFreeze_Key", "按 键").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
                modeLaneFreeze.AddItem(new MenuItem("LaneFreeze_move", "移 动").SetValue(true));
                modeLaneFreeze.AddItem(new MenuItem("LaneFreeze_attack", "攻 击").SetValue(true));
                menuModes.AddSubMenu(modeLaneFreeze);

                var modeLasthit = new Menu("最后 一下", "lxOrbwalker_Modes_LastHit");
                modeLasthit.AddItem(new MenuItem("LastHit_Key", "按 键").SetValue(new KeyBind("X".ToCharArray()[0], KeyBindType.Press)));
                modeLasthit.AddItem(new MenuItem("LastHit_move", "移 动").SetValue(true));
                modeLasthit.AddItem(new MenuItem("LastHit_attack", "攻 击").SetValue(true));
                menuModes.AddSubMenu(modeLasthit);

                var modeFlee = new Menu("逃 生", "lxOrbwalker_Modes_Flee");
                modeFlee.AddItem(new MenuItem("Flee_Key", "按 键").SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                menuModes.AddSubMenu(modeFlee);
            }
            menu.AddSubMenu(menuModes);
            menu.AddItem(new MenuItem("lx_info", "作者： Lexxes OX走砍丨加載成功！漢化by 花边！QQ群|361630847!"));
