        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
          ...
          Menu = new Menu("DZ191 德莱文", "DravenReloaded", true);
            ...
            Menu.AddSubMenu(ts);
             ...
             Menu.AddSubMenu(new Menu("[Draven]连招", "Combo"));
            ...
            Menu.SubMenu("Combo").AddItem(new MenuItem("QManaC", "Q 连招魔法限制").SetValue(new Slider(30, 1, 100)));
            Menu.SubMenu("Combo").AddItem(new MenuItem("WManaC", "W 连招魔法限制").SetValue(new Slider(25, 1, 100)));
            Menu.SubMenu("Combo").AddItem(new MenuItem("EManaC", "E 连招魔法限制").SetValue(new Slider(20, 1, 100)));
            Menu.SubMenu("Combo").AddItem(new MenuItem("RManaC", "R 连招魔法限制").SetValue(new Slider(5, 1, 100)));
            Menu.SubMenu("Combo").AddItem(new MenuItem("CAC", "自动接斧头").SetValue(true));

            Menu.AddSubMenu(new Menu("[Draven]骚扰", "Harrass"));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("UseQH", "使用 Q 骚扰").SetValue(true));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("UseWH", "使用 W 骚扰").SetValue(true));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("UseEH", "使用 E 骚扰").SetValue(true));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("UseRH", "使用 R 骚扰").SetValue(true));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("QManaH", "Q 骚扰魔法限制").SetValue(new Slider(30, 1, 100)));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("WManaH", "W 骚扰魔法限制").SetValue(new Slider(25, 1, 100)));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("EManaH", "E 骚扰魔法限制").SetValue(new Slider(20, 1, 100)));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("RManaH", "R 骚扰魔法限制").SetValue(new Slider(5, 1, 100)));
            Menu.SubMenu("Harrass").AddItem(new MenuItem("CAH", "自动接斧头").SetValue(true));

            Menu.AddSubMenu(new Menu("[Draven]清兵", "Farm"));
            Menu.SubMenu("Farm").AddItem(new MenuItem("UseQLC", "使用 Q 清兵").SetValue(true));
            Menu.SubMenu("Farm").AddItem(new MenuItem("UseELC", "使用 E 清兵").SetValue(true));
            Menu.SubMenu("Farm").AddItem(new MenuItem("QManaLC", "Q 清兵魔法限制").SetValue(new Slider(25, 1, 100)));
            Menu.SubMenu("Farm").AddItem(new MenuItem("EManaLC", "E 清兵魔法限制").SetValue(new Slider(25, 1, 100)));
            Menu.SubMenu("Farm").AddItem(new MenuItem("CAF", "自动接斧头").SetValue(true));

            Menu.AddSubMenu(new Menu("[Draven]斧头设置", "Axes"));
            Menu.SubMenu("Axes").AddItem(new MenuItem("MaxAxeC", "连招接斧头次数").SetValue(new Slider(2, 1, 4)));
            Menu.SubMenu("Axes").AddItem(new MenuItem("MaxAxeH", "骚扰接斧头次数").SetValue(new Slider(2, 1, 4)));
            Menu.SubMenu("Axes").AddItem(new MenuItem("MaxAxeF", "清兵接斧头次数").SetValue(new Slider(2, 1, 4)));
            Menu.SubMenu("Axes")
                .AddItem(new MenuItem("CatchRadius", "接斧头半径").SetValue(new Slider(600, 200, 1000)));
            Menu.SubMenu("Axes")
                .AddItem(new MenuItem("SafeZone", "接斧头安全区域").SetValue(new Slider(125, 0, 325)));
            Menu.AddSubMenu(new Menu("[Draven]杂项", "Misc"));
            Menu.SubMenu("Misc").AddItem(new MenuItem("ManualR", "大招指示").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            ...
            Menu.SubMenu("Items").AddItem(new MenuItem("BotrkH", "骚扰使用破败").SetValue(false));
            Menu.SubMenu("Items").AddItem(new MenuItem("YoumuuC", "连招使用幽梦").SetValue(true));
            Menu.SubMenu("Items").AddItem(new MenuItem("YoumuuH", "骚扰使用幽梦").SetValue(false));
            Menu.SubMenu("Items").AddItem(new MenuItem("BilgeC", "连招使用小弯刀").SetValue(true));
            Menu.SubMenu("Items").AddItem(new MenuItem("BilgeH", "骚扰使用小弯刀").SetValue(false));
            Menu.SubMenu("Items").AddItem(new MenuItem("OwnHPercBotrk", "使用破败自己血量").SetValue(new Slider(50, 1, 100)));
            Menu.SubMenu("Items").AddItem(new MenuItem("EnHPercBotrk", "使用破败敌方血量").SetValue(new Slider(20, 1, 100)));

            Menu.AddSubMenu(new Menu("[Draven]水银设置", "QSSMenu"));
            Menu.SubMenu("QSSMenu").AddItem(new MenuItem("UseQSS", "使用水银弯刀").SetValue(true));
            Menu.AddSubMenu(new Menu("[Draven]水银解除状态", "QSST"));
            Cleanser.CreateTypeQSSMenu();
            Menu.AddSubMenu(new Menu("[Draven]水银解除法术", "QSSSpell"));
            ...
            Menu.SubMenu("Drawing").AddItem(new MenuItem("DrawNextRet", "显示斧头落地点").SetValue(new Circle(true, Color.Orange)));
            Menu.AddToMainMenu();
            ...
            Game.PrintChat("<font color='#FF0000'>DZ|寰疯幈鏂噟</font> 鍔犺浇鎴愬姛!姹夊寲by浜岀嫍!QQ缇361630847");
