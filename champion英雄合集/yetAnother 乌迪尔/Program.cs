...
public class yetAnotherUdyr
{
    ...
    public static void Game_onGameLoad(System.EventArgs args)
    {
        ...
        //Main Menu
        Config = new Menu("yA-乌迪尔", "yA-Udyr", true);
        ...
        //Main
        Config.AddItem(new MenuItem("Combo Key", "连招 按键").SetValue(new KeyBind(32, KeyBindType.Press)));
        Config.AddItem(new MenuItem("Style", "风 格").SetValue(new StringList(new[] { "Phoenix", "Tiger" }, 0)));

        //Items
        Config.AddSubMenu(new Menu("项 目", "Items"));
        //'Offensive
        Config.SubMenu("Items").AddSubMenu(new Menu("进 攻", "Offense"));
        Config.SubMenu("Items").SubMenu("Offense").AddItem(new MenuItem("BilgeCut", "水银弯刀").SetValue(true));
        Config.SubMenu("Items").SubMenu("Offense").AddItem(new MenuItem("BoTRK", "智能能 破败王者之刃").SetValue(true));
        ...
        //'Defensive
        Config.SubMenu("Items").AddSubMenu(new Menu("防 御", "Defense"));
        Config.SubMenu("Items").SubMenu("Defense").AddSubMenu(new Menu("LoT Iron Solari", "LoTIS-菜单"));
        ///LoT-IS
        Config.SubMenu("Items").SubMenu("Defense").SubMenu("LoTIS-Menu").AddItem(new MenuItem("LoTIS", "启用").SetValue(true));
        Config.SubMenu("Items").SubMenu("Defense").SubMenu("LoTIS-Menu").AddItem(new MenuItem("LoTIS-HP-%", "HP % 启用").SetValue(new Slider(40)));

        //Farm
        Config.AddSubMenu(new Menu("打 钱", "Farm"));
        Config.SubMenu("Farm").AddItem(new MenuItem("Use-Q-Farm", "使用 Q").SetValue(true));
        Config.SubMenu("Farm").AddItem(new MenuItem("Use-R-Farm", "使用 R").SetValue(true));
        Config.SubMenu("Farm").AddItem(new MenuItem("Farm-Mana", "蓝量 限制").SetValue(new Slider(20)));
        Config.SubMenu("Farm").AddItem(new MenuItem("Farm Key", "按 键").SetValue(new KeyBind(86, KeyBindType.Press)));
        ...
        Config.SubMenu("Jungle Farm").AddItem(new MenuItem("Use-R-Jungle", "使用 R").SetValue(true));
        ...
        Config.SubMenu("Jungle Farm").AddItem(new MenuItem("Jungle Farm Key", "按 键").SetValue(new KeyBind(67, KeyBindType.Press)));

        //Misc
        Config.AddSubMenu(new Menu("杂 项", "Misc"));
        Config.SubMenu("Misc").AddItem(new MenuItem("Auto Level", "自动 加点").SetValue(true));
        Config.SubMenu("Misc").AddItem(new MenuItem("Stun Lock", "锁定 眩晕").SetValue(true));

        //Drawing
        Config.AddSubMenu(new Menu("范 围", "Drawing"));
        Config.SubMenu("Drawing").AddItem(new MenuItem("Draw", "开 启").SetValue(true));
        //'Enemy Status
        foreach (Obj_AI_Hero enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.IsEnemy))
        {
            ...
            Config.SubMenu("Drawing").SubMenu(enemy.ChampionName).AddItem(new MenuItem(enemy.NetworkId + "E", "开 启").SetValue(true));
            Config.SubMenu("Drawing").SubMenu(enemy.ChampionName).AddItem(new MenuItem(enemy.NetworkId + "KC", "击杀 线圈").SetValue(true));
            ...
            Config.SubMenu("Drawing").SubMenu(enemy.ChampionName).AddItem(new MenuItem(enemy.NetworkId + "R", "范 围").SetValue(new StringList(new[] { "Basic", "Q", "W", "E", "R" })));
            Config.SubMenu("Drawing").SubMenu(enemy.ChampionName).AddItem(new MenuItem(enemy.NetworkId + "RC", "范围 颜色").SetValue(new Circle(true, System.Drawing.Color.Gray)));
        }
