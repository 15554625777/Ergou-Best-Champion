...
namespace LightningLux
{
	internal class Program
	{
		...
		private static void Game_OnGameLoad(EventArgs args)
		{
			...
			Config = new Menu("Lightning 光辉", "Lightning Lux", true);
			...
			Config.SubMenu("Combo").AddItem(new MenuItem("UseR", "可击杀对方时使用 R").SetValue(true));
			Config.SubMenu("Combo").AddItem(new MenuItem("UseItems", "使用 项目").SetValue(true));
			...
			Config.SubMenu("Farm").AddItem(new MenuItem("JungSteal", "打野 gank!").SetValue(new KeyBind("J".ToCharArray()[0], KeyBindType.Press)));
			...
			Config.AddSubMenu(new Menu("自动 W", "AutoShield"));
			Config.SubMenu("AutoShield").AddItem(new MenuItem("WAllies", "自动给w 盟友").SetValue(true));
			Config.SubMenu("AutoShield").AddItem(new MenuItem("AutoW", "自己被针对时自动 W").SetValue(true));
			...
			Config.SubMenu("ExtraSettings").AddItem(new MenuItem("UseQE", "只有敌人被困住时使用 E").SetValue(false));
			Config.SubMenu("ExtraSettings").AddItem(new MenuItem("AutoE2", "自动扔 E").SetValue(true));
			Config.SubMenu("ExtraSettings").AddItem(new MenuItem("UseQGap", "被突进时自动 Q").SetValue(true));
			Config.SubMenu("ExtraSettings").AddItem(new MenuItem("HitChance", "打击 机会").SetValue(new StringList(new[] {"Low","Medium","High","Very High"},2)));
			Config.SubMenu("ExtraSettings").AddItem(new MenuItem("UsePacket", "使用 封包").SetValue(true));
			
			Config.AddSubMenu(new Menu("大招 设置", "UltSettings"));
			Config.SubMenu("UltSettings").AddItem(new MenuItem("RHit", "能命中时 自动r").SetValue(new StringList(new[] {"None","2 Target","3 Target","4 Target","5 Target"},1)));
			Config.SubMenu("UltSettings").AddItem(new MenuItem("RTrap", "对面被困住 自动r").SetValue(false));
			...
			Config.SubMenu("Drawings").AddItem(new MenuItem("WRange", "W 范 围").SetValue(new Circle(false, Color.FromArgb(255, 255, 255, 255))));
			Config.SubMenu("Drawings").AddItem(new MenuItem("ERange", "E 范 围").SetValue(new Circle(false, Color.FromArgb(255, 255, 255, 255))));
			Config.AddToMainMenu();
			
			Game.PrintChat("Lightning 鎷夊厠涓濅辅鍔犺級鎴愬姛锛佹饥鍖朾y 鑺辫竟锛丵Q缇361630847!");
