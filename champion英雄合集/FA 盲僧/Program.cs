...
namespace FuckingAwesomeLeeSin
{
    class Program
    {
        ...
        static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Menu.SubMenu("Combo").AddItem(new MenuItem("useW", "连招使用瞬眼").SetValue(false));
            Menu.SubMenu("Combo").AddItem(new MenuItem("dsjk", "使用顺眼|如果范围: "));
            Menu.SubMenu("Combo").AddItem(new MenuItem("wMode", "> AA范围|| > Q范围").SetValue(true));
            ...
            insecMenu.AddItem(new MenuItem("insec2tower", "R向炮塔").SetValue(true));
            insecMenu.AddItem(new MenuItem("bonusRangeT", "R向炮塔范围").SetValue(new Slider(0, 0, 1000)));
            ...
            insecMenu.AddItem(new MenuItem("instaFlashInsec2", "使用闪现回旋踢"));
            insecMenu.AddItem(new MenuItem("instaFlashInsec", "启用").SetValue(new KeyBind("P".ToCharArray()[0], KeyBindType.Toggle)));
            Menu.AddSubMenu(insecMenu);

            var autoSmiteSettings = new Menu("惩戒 设置", "Auto Smite Settings");
            autoSmiteSettings.AddItem(new MenuItem("smiteEnabled", "启用").SetValue(new KeyBind("M".ToCharArray()[0], KeyBindType.Toggle)));
            ...
            SaveMeMenu.AddItem(new MenuItem("smiteSave", "启用 节省惩戒").SetValue(true));
            SaveMeMenu.AddItem(new MenuItem("hpPercentSM", "WW惩戒 on x%").SetValue(new Slider(10, 1)));
            SaveMeMenu.AddItem(new MenuItem("param1", "禁用惩戒|血量设定")); // TBC
            SaveMeMenu.AddItem(new MenuItem("dBuffs", "红蓝Buffs").SetValue(true));// TBC
            SaveMeMenu.AddItem(new MenuItem("hpBuffs", "禁用惩戒|血量HP %").SetValue(new Slider(30, 1)));// TBC
            SaveMeMenu.AddItem(new MenuItem("dEpics", "野怪").SetValue(true));// TBC
            SaveMeMenu.AddItem(new MenuItem("hpEpics", "禁用惩戒|血量HP %").SetValue(new Slider(10, 1)));// TBC
            Menu.AddSubMenu(SaveMeMenu);
            //Wardjump menu
            var wardjumpMenu = new Menu("瞬眼", "Wardjump");
            wardjumpMenu.AddItem(
                new MenuItem("wjump", "瞬眼 键位").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Press)));
            wardjumpMenu.AddItem(new MenuItem("maxRange", "总是瞬眼极限范围").SetValue(false));
            wardjumpMenu.AddItem(new MenuItem("castInRange", "向鼠标位置瞬眼").SetValue(false));
            wardjumpMenu.AddItem(new MenuItem("m2m", "移动到鼠标位置再瞬眼").SetValue(true));
            wardjumpMenu.AddItem(new MenuItem("j2m", "瞬眼到小兵").SetValue(true));
            wardjumpMenu.AddItem(new MenuItem("j2c", "瞬眼到敌人").SetValue(true));
