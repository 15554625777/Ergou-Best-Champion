...
//                    .SetValue(ObjectManager.Player.Spellbook.GetSpell(SpellSlot.R).Name == "LucianR");
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseQTH" + Id, "使用 Q (自動)").SetValue(new KeyBind("T".ToCharArray()[0],
                    KeyBindType.Toggle)));
            config.AddItem(new MenuItem("Cx",""));
            config.AddItem(new MenuItem("UseQExtendedH" + Id, "使用 延伸 Q").SetValue(true));
            config.AddItem(
                new MenuItem("UseQExtendedTH" + Id, "使用 延伸 Q (自动)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
            config.AddItem(new MenuItem("Cx", ""));
        ...
        {
            config.AddItem(new MenuItem("Passive" + Id, "检测 被动").SetValue(true));
            return true;
        }

        public override bool DrawingMenu(Menu config)
        {
            config.AddItem(new MenuItem("DrawQ" + Id, "Q 範圍").SetValue(new Circle(true, Color.Gray)));
            config.AddItem(new MenuItem("DrawQ2" + Id, "延伸 Q 范围").SetValue(new Circle(true, Color.Gray)));
            config.AddItem(new MenuItem("DrawW" + Id, "W 範圍").SetValue(new Circle(false, Color.Gray)));
            config.AddItem(new MenuItem("DrawE" + Id, "E 範圍").SetValue(new Circle(false, Color.Gray)));
