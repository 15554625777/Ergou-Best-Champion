...
namespace Marksman
{
    internal class Jinx : Champion
    {
        ...
        {
            config.AddItem(new MenuItem("SwapQ" + Id, "总是切换机枪").SetValue(false));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("SwapDistance" + Id, "远距离攻击自动切换Q").SetValue(true));
            config.AddItem(new MenuItem("SwapAOE" + Id, "切换Q AOE").SetValue(false));
            config.AddItem(new MenuItem("MinWRange" + Id, "W最小范围").SetValue(new Slider(525 + 65 * 2, 0, 1200)));
            config.AddItem(new MenuItem("AutoEI" + Id, "自动-E 不动的").SetValue(true));
            config.AddItem(new MenuItem("AutoES" + Id, "自动-E 缓慢的").SetValue(true));
            config.AddItem(new MenuItem("AutoED" + Id, "自动-E 疾走的").SetValue(false));
            config.AddItem(
                new MenuItem("CastR" + Id, "手動 R (2000 範圍)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            config.AddItem(new MenuItem("ROverKill" + Id, "R-击杀提示").SetValue(true));
        ...
        {
            config.AddItem(
                new MenuItem("DrawQBound" + Id, "显示 Q 受限距离").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 0))));
            config.AddItem(
                new MenuItem("DrawW" + Id, "W 範圍").SetValue(new Circle(false, Color.CornflowerBlue)));
            return true;
        }
