...
namespace Marksman
{
    internal class Urgot : Champion
    {
        ...
        public override bool ComboMenu(Menu config)
        {
            ...
            config.AddSubMenu(new Menu("大招設置 1", "UltOpt1"));
            ...
            config.AddSubMenu(new Menu("大招設置 2", "UltOpt2"));
            config.SubMenu("UltOpt2")
                .AddItem(
                    new MenuItem("UltOp2" + Id, "R到友方陣容").SetValue(new KeyBind("G".ToCharArray()[0],
                        KeyBindType.Press)));
            config.SubMenu("UltOpt2")
                .AddItem(new MenuItem("UltOp2Count" + Id, "使用R|身边最少队友").SetValue(new Slider(1, 1, 5)));


            config.AddSubMenu(new Menu("不使用 Ult on", "DontUlt"));
        ...
        {
            config.AddItem(
                new MenuItem("DrawQ" + Id, "Q 範圍").SetValue(new Circle(true, Color.LightGray)));
            config.AddItem(
                new MenuItem("DrawE" + Id, "E 範圍").SetValue(new Circle(false, Color.LightGray)));
            config.AddItem(
                new MenuItem("DrawR" + Id, "R 範圍").SetValue(new Circle(false, Color.LightGray)));
            config.AddItem(
