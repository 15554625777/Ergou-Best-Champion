...
namespace Marksman
{
    internal class Tristana : Champion
    {
        ...
        {
            config.AddItem(
                new MenuItem("DrawE" + Id, "E 範圍").SetValue(new Circle(true, Color.CornflowerBlue)));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("UseRM" + Id, "使用 R 搶人頭").SetValue(true));
            config.AddItem(new MenuItem("UseRMG" + Id, "使用 R 防止突進").SetValue(true));
            config.AddItem(new MenuItem("UseRMI" + Id, "使用 R 中斷法術").SetValue(true));
            return true;
        }
