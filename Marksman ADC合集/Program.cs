...
namespace Marksman
{
    internal class Program
    {
        ...
        {
            Config = new Menu("【嬉皮笑臉】ADC英雄合集", "Marksman", true);
            CClass = new Champion();
            ...
            /* Menu Summoners */
            var summoners = Config.AddSubMenu(new Menu("召喚師技能", "Summoners"));
            var summonersHeal = summoners.AddSubMenu(new Menu("治愈", "Heal"));
            {
                summonersHeal.AddItem(new MenuItem("SUMHEALENABLE", "啟用").SetValue(true));
                summonersHeal.AddItem(new MenuItem("SUMHEALSLIDER", "使用治愈丨最低血量").SetValue(new Slider(20, 99, 1)));
            }

            var summonersBarrier = summoners.AddSubMenu(new Menu("屏障", "Barrier"));
            {
                summonersBarrier.AddItem(new MenuItem("SUMBARRIERENABLE", "啟用").SetValue(true));
                summonersBarrier.AddItem(
                    new MenuItem("SUMBARRIERSLIDER", "使用屏障丨最低血量").SetValue(new Slider(20, 99, 1)));
            }

            var summonersIgnite = summoners.AddSubMenu(new Menu("Ignite", "點燃"));
            {
                summonersIgnite.AddItem(new MenuItem("SUMIGNITEENABLE", "啟用").SetValue(true));
            }
            /* Menu Items */            
            var items = Config.AddSubMenu(new Menu("物品", "Items"));
            items.AddItem(new MenuItem("BOTRK", "破敗").SetValue(true));
            items.AddItem(new MenuItem("GHOSTBLADE", "小破敗").SetValue(true));
            ...
            QuickSilverMenu = new Menu("水銀飾帶", "QuickSilverSash");
            items.AddSubMenu(QuickSilverMenu);
            QuickSilverMenu.AddItem(new MenuItem("AnyStun", "解除眩暈").SetValue(true));
            QuickSilverMenu.AddItem(new MenuItem("AnySlow", "解除減速").SetValue(true));
            QuickSilverMenu.AddItem(new MenuItem("AnySnare", "解除陷阱").SetValue(true));
            QuickSilverMenu.AddItem(new MenuItem("AnyTaunt", "解除嘲諷").SetValue(true));
            ...
            items.AddItem(
                new MenuItem("UseItemsMode", "使用物品").SetValue(
                    new StringList(new[] {"禁用", "混合", "連招", "全部"}, 2)));
            ...
            if (BaseType != CClass.GetType())
            {
                ...
                {
                    harass.AddItem(new MenuItem("HarassMana", "騷擾最低藍量").SetValue(new Slider(50, 100, 0)));
                    Config.AddSubMenu(harass);
                }

                var laneclear = new Menu("清線", "LaneClear");
                if (CClass.LaneClearMenu(laneclear))
                {
                    laneclear.AddItem(
                        new MenuItem("LaneClearMana", "清線最低藍量").SetValue(new Slider(50, 100, 0)));
                ...
                var drawing = new Menu("範圍", "Drawings");
                if (CClass.DrawingMenu(drawing))
                {
                    drawing.AddItem(
                        new MenuItem("drawMinionLastHit", "標記補刀小兵").SetValue(new Circle(false,
                            System.Drawing.Color.GreenYellow)));
                    drawing.AddItem(
                        new MenuItem("drawMinionNearKill", "標記附近可擊殺小兵").SetValue(new Circle(false,
