...
namespace Marksman
{
    internal class Caitlyn : Champion // Base done by xQx, Drawings and improvements added by Dibes.
    {
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("DrawUlt" + Id, "大招傷害（文本）").SetValue(new Circle(true, Color.FromArgb(255, 255, 255, 255))));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("UltHelp" + Id, "對攻擊目標使用R").SetValue(new KeyBind("R".ToCharArray()[0],
                    KeyBindType.Press)));
            config.AddItem(
                new MenuItem("UseEQC" + Id, "使用 E-Q 连招").SetValue(new KeyBind("T".ToCharArray()[0],
                    KeyBindType.Press)));
            config.AddItem(
                new MenuItem("Dash" + Id, "E 向鼠标位置").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
            return true;
        }
