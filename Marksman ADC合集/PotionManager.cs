...
namespace Marksman
{
    internal class PotionManager
    {
        ...
        private void Load()
        {
            ...
            ExtrasMenu.AddSubMenu(new Menu("藥水 大師", "PotionManager"));

            ExtrasMenu.SubMenu("PotionManager").AddSubMenu(new Menu("紅藥", "Health"));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Health")
                .AddItem(new MenuItem("HealthPotion", "使用 红药").SetValue(true));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Health")
                .AddItem(new MenuItem("HealthPercent", "使用红药最低血量").SetValue(new Slider(30)));

            ExtrasMenu.SubMenu("PotionManager").AddSubMenu(new Menu("藍藥", "Mana"));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Mana")
                .AddItem(new MenuItem("ManaPotion", "使用 蓝药 Potion").SetValue(true));
            ExtrasMenu.SubMenu("PotionManager")
                .SubMenu("Mana")
                .AddItem(new MenuItem("ManaPercent", "使用蓝药最低魔量").SetValue(new Slider(30)));
