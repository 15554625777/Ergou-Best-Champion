...
namespace Marksman
{
    ...
    internal class Draven : Champion
    {
        ...
        {
            config.AddItem(
                new MenuItem("DrawE", "E 範圍").SetValue(new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            config.AddItem(
                new MenuItem("DrawR", "R 範圍(2000 範圍)").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            config.AddItem(
                new MenuItem("DrawOrbwalk", "显示走砍（接斧头）位置").SetValue(new Circle(true, Color.Yellow)));
            config.AddItem(new MenuItem("DrawReticles", "显示斧头落地十字线").SetValue(new Circle(true, Color.Green)));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("maxqamount", "最高接斧头次数").SetValue(new Slider(2, 4, 1)));
            config.AddItem(new MenuItem("EGapCloser", "自动E防止突进").SetValue(true));
            config.AddItem(new MenuItem("EInterruptable", "自动E中断法术").SetValue(true));
            config.AddItem(new MenuItem("RManualCast", "手动R(2000 范围)"))
                .SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press));
            config.AddItem(new MenuItem("Epeel", "使用E减速").SetValue(true));
            config.AddItem(new MenuItem("CatchRadius", "接斧头半径").SetValue(new Slider(600, 200, 1000)));
            return true;
        }
