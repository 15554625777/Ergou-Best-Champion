...
namespace Marksman
{
    internal class Vayne : Champion
    {
        ...
        {
            config.AddItem(
                new MenuItem("UseET" + Id, "使用 E (自動)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
            config.AddItem(new MenuItem("UseEInterrupt" + Id, "使用 E 中斷法術").SetValue(true));
            config.AddItem(
                new MenuItem("PushDistance" + Id, "使用E|敌人和自己的距离").SetValue(new Slider(425, 475, 300)));
            config.AddItem(new MenuItem("CompleteSilverBuff" + Id, "完整的使用QA").SetValue(true));
            return true;
        }
