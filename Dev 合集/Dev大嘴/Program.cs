    ...
    class Program
    {
        ...
        private static void onGameLoad(EventArgs args)
        {
            try
            {
                ...
                Game.PrintChat(string.Format("<font color='#fb762d'>Dev澶у槾 鍔犺級鎴愬姛! v{0}姹夊寲by浜岀嫍!QQ缇361630847</font>", Assembly.GetExecutingAssembly().GetName().Version));
        ...
        private static void InitializeMainMenu()
        {
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("RMaxStacksCombo", "R最大使用次数").SetValue(new Slider(3, 1, 5)));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseIgnite", "使用点燃").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("EManaCombo", "使用E最低蓝量").SetValue(new Slider(20, 1, 100)));
            ...
            Config.SubMenu("Harass").AddItem(new MenuItem("UseRHarass", "使用 R").SetValue(true));
            Config.SubMenu("Harass").AddItem(new MenuItem("RMaxStacksHarass", "R最大使用次数").SetValue(new Slider(1, 1, 5)));
            Config.SubMenu("Harass").AddItem(new MenuItem("ManaHarass", "骚扰最低蓝量").SetValue(new Slider(25, 1, 100)));
            Config.SubMenu("Harass").AddItem(new MenuItem("EManaHarass", "使用E最低蓝量").SetValue(new Slider(60, 1, 100)));
            
            Config.AddSubMenu(new Menu("清线", "LaneClear"));
            Config.SubMenu("LaneClear").AddItem(new MenuItem("UseELaneClear", "使用 E").SetValue(false));
            Config.SubMenu("LaneClear").AddItem(new MenuItem("EManaLaneClear", "使用E最低蓝量").SetValue(new Slider(50, 1, 100)));

            Config.AddSubMenu(new Menu("抢人头|", "KillSteal"));
            Config.SubMenu("KillSteal").AddItem(new MenuItem("RKillSteal", "使用R抢人头").SetValue(true));

            Config.AddSubMenu(new Menu("抢野（大小龙等）", "JungleSteal"));
            Config.SubMenu("JungleSteal").AddItem(new MenuItem("JungleStealAlert", "抢野 警觉").SetValue(true));
            Config.SubMenu("JungleSteal").AddItem(new MenuItem("JungleSteal", "使用R抢野").SetValue(true));

            Config.AddSubMenu(new Menu("杂项", "Misc"));
            Config.SubMenu("Misc").AddItem(new MenuItem("ChaseEnemyAfterDeath", "自动死亡追逐").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("PacketCast", "使用封包").SetValue(true));

            Config.AddSubMenu(new Menu("突进终结者|", "GapCloser"));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("BarrierGapCloser", "阻挡突进者|").SetValue(true));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("BarrierGapCloserMinHealth", "阻挡突进者|最低血量").SetValue(new Slider(40, 0, 100)));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("EGapCloser", "使用E防止突进").SetValue(true));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("EInterrupt", "使用E中断法术").SetValue(true));

            Config.AddSubMenu(new Menu("R设置", "Ultimate"));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UseAssistedUlt", "使用R支援队友").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("AssistedUltKey", "R支援键位").SetValue((new KeyBind("R".ToCharArray()[0], KeyBindType.Press))));
            ...
            Config.SubMenu("Drawings").AddItem(new MenuItem("RDamage", "显示R血量伤害").SetValue(true));
