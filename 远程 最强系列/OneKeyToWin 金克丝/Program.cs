...
namespace Jinx
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config = new Menu("OneKeyToWin 金克丝", "Jinx", true);
            ...
            #region E
                Config.SubMenu("E Config").AddItem(new MenuItem("autoE", "连招自动E").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("AGC", "自动E 防止突进").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("opsE", "自动E 中断法术").SetValue(true));
            #endregion
            #region R
                Config.SubMenu("R Config").AddItem(new MenuItem("autoR", "连招自动R").SetValue(true));
                Config.SubMenu("R Config").AddItem(new MenuItem("hitchanceR", "R命中率：非常高").SetValue(true));
                Config.SubMenu("R Config").AddItem(new MenuItem("useR", "手动 R 键位").SetValue(new KeyBind('t', KeyBindType.Press))); //32 == space
            #endregion
            ...
            Game.PrintChat("<font color=\"#ff00d8\">OneKeyToWin |閲戝厠涓潀</font>2.5.1 <font color=\"#000000\">by sebastiank1</font> - <font color=\"#00BFFF\">鍔犺級鎴愬姛锛佹饥鍖朾y浜岀嫍锛丵Q缇361630847</font>");
        }
