...
namespace D_Graves
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            _config.SubMenu("Combo").AddItem(new MenuItem("diveintower", "使用E冲进塔内")).SetValue(true);
            ...
            _config.SubMenu("R Factions").AddSubMenu(new Menu("Use R combo", "使用 R 连招"));
            _config.SubMenu("R Factions")
                .SubMenu("Use R combo")
                .AddItem(new MenuItem("UseRcombo", "连招 使用 R"))
                .SetValue(true);
            _config.SubMenu("R Factions")
                .SubMenu("Use R combo")
                .AddItem(new MenuItem("UseRrush", "使用R 如果连招伤害>=目标 HP"))
                .SetValue(true);
            _config.SubMenu("R Factions")
                .SubMenu("Use R combo")
                .AddItem(new MenuItem("UseRC", "使用R 如果R伤害>目标. HP"))
                .SetValue(true);
            _config.SubMenu("R Factions")
                .SubMenu("Use R combo")
                .AddItem(new MenuItem("UseRE", "自动R|敌人数量"))
                .SetValue(true);
            _config.SubMenu("R Factions")
                .SubMenu("Use R combo")
                .AddItem(new MenuItem("MinTargets", "自动R|敌人数量").SetValue(new Slider(2, 1, 5)));
            _config.SubMenu("R Factions").SubMenu("Use R combo").AddItem(new MenuItem("", "目标人数低于丨使用R"));
            ...
            _config.SubMenu("R Factions").AddSubMenu(new Menu("使用 R 抢人头", "Use R killsteal"));
            _config.SubMenu("R Factions")
                .SubMenu("Use R killsteal")
                .AddItem(new MenuItem("UseRM", "使用 R 抢人头"))
                .SetValue(true);
            _config.SubMenu("R Factions").SubMenu("Use R killsteal").AddItem(new MenuItem("", "目标血量低于丨使用R"));
            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => enemy.Team != _player.Team))
                _config.SubMenu("R Factions")
                    .SubMenu("Use R killsteal")
            ...
            _config.SubMenu("Farm").AddItem(new MenuItem("UseQL", "Q 补兵")).SetValue(true);
            _config.SubMenu("Farm").AddItem(new MenuItem("UseWL", "W 补兵")).SetValue(true);
            _config.SubMenu("Farm").AddItem(new MenuItem("UseQLH", "Q 清线")).SetValue(true);
            _config.SubMenu("Farm").AddItem(new MenuItem("UseWLH", "W 清线")).SetValue(true);
            ...
            _config.SubMenu("Farm").AddItem(new MenuItem("Lanemana", "清野丨清兵蓝量").SetValue(new Slider(60, 1, 100)));
            _config.SubMenu("Farm")
                .AddItem(
                    new MenuItem("ActiveLast", "补刀!").SetValue(new KeyBind("X".ToCharArray()[0], KeyBindType.Press)));
            _config.SubMenu("Farm")
                .AddItem(
                    new MenuItem("ActiveLane", "清野丨清线!").SetValue(new KeyBind("V".ToCharArray()[0],
                        KeyBindType.Press)));
            ...
            _config.SubMenu("items")
                .SubMenu("Offensive")
                .AddItem(new MenuItem("Blademyhp", "使用破败丨敌人血量").SetValue(new Slider(85, 1, 100)));
            _config.SubMenu("items").AddSubMenu(new Menu("恢复物品", "Potions"));
            _config.SubMenu("items")
                .SubMenu("Potions")
            ...
            _config.SubMenu("items")
                .SubMenu("Potions")
                .AddItem(new MenuItem("usepotionhp", "血量低于丨使用红药").SetValue(new Slider(35, 1, 100)));
            _config.SubMenu("items")
                .SubMenu("Potions")
            ...
            _config.SubMenu("items")
                .SubMenu("Potions")
                .AddItem(new MenuItem("usepotionmp", "蓝量低于丨使用蓝药").SetValue(new Slider(35, 1, 100)));

            //Misc
            _config.AddSubMenu(new Menu("杂项", "Misc"));
            _config.SubMenu("Misc").AddItem(new MenuItem("UseQM", "使用 Q 抢人头")).SetValue(true);
            _config.SubMenu("Misc").AddItem(new MenuItem("UseWM", "使用 W 抢人头")).SetValue(true);
            _config.SubMenu("Misc").AddItem(new MenuItem("Gap_E", "自动 W 突脸")).SetValue(true);
            _config.SubMenu("Misc").AddItem(new MenuItem("skinG", "使用换肤功能").SetValue(true));
            _config.SubMenu("Misc").AddItem(new MenuItem("skinGraves", "选择 皮肤").SetValue(new Slider(4, 1, 6)));
            ...
            _config.SubMenu("HitChance")
                .SubMenu("Combo")
                .AddItem(
                    new MenuItem("Qchange", "Q 命中率").SetValue(
                        new StringList(new[] { "低", "中", "高", "很高" })));
            _config.SubMenu("HitChance")
                .SubMenu("Combo")
                .AddItem(
                    new MenuItem("Wchange", "W 命中率").SetValue(
                        new StringList(new[] { "低", "中", "高", "很高" })));
            _config.SubMenu("HitChance")
                .SubMenu("Combo")
                .AddItem(
                    new MenuItem("Rchange", "R 命中率").SetValue(
                        new StringList(new[] { "低", "中", "高", "很高" })));
            _config.SubMenu("HitChance").AddSubMenu(new Menu("骚扰", "Harass"));
            _config.SubMenu("HitChance")
                .SubMenu("Harass")
                .AddItem(
                    new MenuItem("Qchangeharass", "Q 命中率").SetValue(
                        new StringList(new[] { "低", "中", "高", "很高" })));
            _config.SubMenu("HitChance")
                .SubMenu("Harass")
                .AddItem(
                    new MenuItem("Wchangeharass", "W 命中率").SetValue(
                        new StringList(new[] { "低", "中", "高", "很高" })));
            _config.SubMenu("HitChance").AddSubMenu(new Menu("抢人头", "KillSteal"));
            _config.SubMenu("HitChance")
                .SubMenu("KillSteal")
                .AddItem(
                    new MenuItem("Qchangekill", "Q 命中率").SetValue(
                        new StringList(new[] { "低", "中", "高", "很高" })));
            _config.SubMenu("HitChance")
                .SubMenu("KillSteal")
                .AddItem(
            ...
            _config.SubMenu("Drawings").AddItem(new MenuItem("CircleLag", "自动延迟圈").SetValue(true));
            ...
            Game.PrintChat("<font color='#881df2'>D-鐢锋灙 鍔犺浇鎴愬姛!姹夊寲by楝肩伒!QQ缇361630847.");
