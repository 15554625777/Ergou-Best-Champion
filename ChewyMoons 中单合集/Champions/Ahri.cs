...
namespace Mid_or_Feed.Champions
{
    internal class Ahri : Plugin
    {
        ...
        {
            config.AddItem(new MenuItem("useQ", "使用 Q").SetValue(true));
            config.AddItem(new MenuItem("useW", "使用 W").SetValue(true));
            config.AddItem(new MenuItem("useE", "使用 E").SetValue(true));
        }

        public override void Harass(Menu config)
        {
            config.AddItem(new MenuItem("useQHarass", "使用 Q").SetValue(true));
        }

        public override void ItemMenu(Menu config)
        {
            config.AddItem(new MenuItem("useDFG", "使用 冥火").SetValue(true));
        }

        public override void Misc(Menu config)
        {
            config.AddItem(new MenuItem("gapcloseE", "E 防止突进").SetValue(true));
            config.AddItem(new MenuItem("interruptE", "E 中断法术", true).SetValue(true));
        }

        public override void Drawings(Menu config)
        {
            config.AddItem(new MenuItem("drawQ", "范围 Q").SetValue(true));
            config.AddItem(new MenuItem("drawW", "范围 W").SetValue(true));
            config.AddItem(new MenuItem("drawE", "范围 E").SetValue(true));
        }
    }
}