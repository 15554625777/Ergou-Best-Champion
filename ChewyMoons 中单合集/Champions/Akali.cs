...
namespace Mid_or_Feed.Champions
{
    internal class Akali : Plugin
    {
        ...
        {
            itemsMenu.AddItem(new MenuItem("useGunblade", "使用 科技枪").SetValue(true));
            itemsMenu.AddItem(new MenuItem("useCutlass", "使用 小弯刀").SetValue(true));
        }

        public override void Misc(Menu miscMenu)
        {
            miscMenu.AddItem(new MenuItem("gapcloseR", "突进使用 R").SetValue(true));
            miscMenu.AddItem(new MenuItem("gapcloseAmmo", "^ 使用突进|R层数").SetValue(new StringList(new[] {"2", "3"})));
        }

        public override void Drawings(Menu drawingMenu)
        {
            drawingMenu.AddItem(new MenuItem("drawQ", "范围 Q").SetValue(new Circle(true, Color.White)));
            drawingMenu.AddItem(new MenuItem("drawE", "范围 E").SetValue(new Circle(true, Color.White)));
            drawingMenu.AddItem(new MenuItem("drawR", "范围 R").SetValue(new Circle(true, Color.White)));
        }
    }
}