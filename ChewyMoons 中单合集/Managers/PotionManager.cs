...
namespace Mid_or_Feed.Managers
{
    class PotionManager : Manager
    {
        ...
        {
            config.AddItem(new MenuItem("useHP", "使用红药").SetValue(true));
            config.AddItem(new MenuItem("useHPPercent", "血量 %").SetValue(new Slider(35, 1)));
            config.AddItem(new MenuItem("sseperator", "       "));
            config.AddItem(new MenuItem("useMP", "使用蓝药").SetValue(true));
            config.AddItem(new MenuItem("useMPPercent", "蓝量 %").SetValue(new Slider(35, 1)));
