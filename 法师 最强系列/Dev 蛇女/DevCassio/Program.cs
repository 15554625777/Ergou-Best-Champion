    ...
    class Program
    {
        ...
        private static void onGameLoad(EventArgs args)
        {
            try
            {
                ...
                Game.PrintChat(string.Format("<font color='#fb762d'>{0} Dev铔囧コ 鍔犺級鎴愬姛锛佹饥鍖朾y浜岀嫍锛丵Q缇361630847 v{1}</font>", Assembly.GetExecutingAssembly().GetName().Name, Assembly.GetExecutingAssembly().GetName().Version));
        ...
        private static void InitializeMainMenu()
        {
            ...
            Config = new Menu("Dev 蛇女", "DevCassio", true);
            ...
            Config.AddSubMenu(new Menu("大招 设置", "Ultimate"));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UseAssistedUlt", "使用大招支援").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("AssistedUltKey", "強制R 鍵位").SetValue((new KeyBind("R".ToCharArray()[0], KeyBindType.Press))));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("BlockUlt", "防止 空大").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UseUltUnderTower", "自动R塔下敌人").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UltRange", "使用 R 范围").SetValue(new Slider(650, 0, 800)));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("RMinHit", "使用 R 目标").SetValue(new Slider(2, 1, 5)));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("RMinHitFacing", "使用 R 目标（面视)").SetValue(new Slider(1, 1, 5)));
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("UseAACombo", "使用 走A").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseRSaveYourself", "使用 R 保护自己").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseRSaveYourselfMinHealth", "使用R保护|自己血量").SetValue(new Slider(25, 0, 100)));
            ...
            Config.AddSubMenu(new Menu("控线 设置", "Freeze"));
            Config.SubMenu("Freeze").AddItem(new MenuItem("UseEFreeze", "使用 E").SetValue(true));

            Config.AddSubMenu(new Menu("清线 设置", "LaneClear"));
            ...
            Config.SubMenu("LaneClear").AddItem(new MenuItem("UseELastHitLaneClear", "使用 E 补刀").SetValue(true));
            Config.SubMenu("LaneClear").AddItem(new MenuItem("UseELastHitLaneClearNonPoisoned", "使用 E 补刀（未中毒单位）").SetValue(false));
            Config.SubMenu("LaneClear").AddItem(new MenuItem("LaneClearMinMana", "清线最低蓝量").SetValue(new Slider(25, 0, 100)));

            Config.AddSubMenu(new Menu("清野 设置", "JungleClear"));
            ...
            Config.AddSubMenu(new Menu("突进 保护", "Gapcloser"));
            Config.SubMenu("Gapcloser").AddItem(new MenuItem("RAntiGapcloser", "使用 R 防止突进").SetValue(true));
            Config.SubMenu("Gapcloser").AddItem(new MenuItem("RInterrupetSpell", "使用 R 中断法术").SetValue(true));
            Config.SubMenu("Gapcloser").AddItem(new MenuItem("RAntiGapcloserMinHealth", "使用R|突进者的血量").SetValue(new Slider(60, 0, 100)));

            Config.AddSubMenu(new Menu("杂项 设置", "Misc"));
            Config.SubMenu("Misc").AddItem(new MenuItem("PacketCast", "不回头施法(使用封包)").SetValue(true));

            Config.AddSubMenu(new Menu("E 设置", "Legit"));
            Config.SubMenu("Legit").AddItem(new MenuItem("PlayLegit", "E 设置:").SetValue(false));
            Config.SubMenu("Legit").AddItem(new MenuItem("DisableNFE", "禁用 封包E").SetValue(true));
            Config.SubMenu("Legit").AddItem(new MenuItem("LegitCastDelay", "使用E延迟").SetValue(new Slider(1000, 0, 2000)));

            Config.AddSubMenu(new Menu("范围 设置", "Drawings"));
            ...
            Config.SubMenu("Drawings").AddItem(new MenuItem("EDamage", "显示E对敌人血量伤害").SetValue(true));
