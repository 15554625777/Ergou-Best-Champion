...
namespace Xerath
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Create the menu
            Config = new Menu("花边汉化-Esk0r泽拉斯", ChampionName, true);
            ...
            Config.SubMenu("R").AddItem(new MenuItem("EnableRUsage", "自动 释放").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("rMode", "模 式:").SetValue(new StringList(new[] { "正常", "自定义 延迟", "优先"})));
            Config.SubMenu("R").AddItem(new MenuItem("rModeKey", "优先 按键").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            ...
            Config.SubMenu("Harass")
                .AddItem(
                    new MenuItem("HarassActive", "开启 骚扰!").SetValue(
            ...
            Config.SubMenu("Farm")
                .AddItem(
                    new MenuItem("UseQFarm", "使用 Q").SetValue(
                        new StringList(new[] { "控 线", "清 线", "两者", "两者都不" }, 2)));
            Config.SubMenu("Farm")
                .AddItem(
                    new MenuItem("UseWFarm", "使用 W").SetValue(
                        new StringList(new[] { "控 线", "清 线", "两者", "两者都不" }, 1)));
            Config.SubMenu("Farm")
                .AddItem(
            ...
            Config.SubMenu("JungleFarm")
                .AddItem(
                    new MenuItem("JungleFarmActive", "开启 清野!").SetValue(
                        new KeyBind(Config.Item("LaneClear").GetValue<KeyBind>().Key, KeyBindType.Press)));

            //Misc
            Config.AddSubMenu(new Menu("杂 项", "Misc"));
            Config.SubMenu("Misc").AddItem(new MenuItem("InterruptSpells", "打断 法术").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("AutoEGC", "被突进时 自动E").SetValue(true));
