...
class SkinManager
{
    ...
    public void AddToMenu(ref Menu menu)
    {
        ...
        if (Skins.Count > 0)
        {
            Menu.AddSubMenu(new Menu("皮肤 更换", "Skin Changer"));
            Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_enabled", "启用 换肤").SetValue(false));
            Menu.SubMenu("Skin Changer").AddItem(new MenuItem("Skin_" + ObjectManager.Player.ChampionName + "_select", "选择 皮肤").SetValue(new StringList(Skins.ToArray())));
