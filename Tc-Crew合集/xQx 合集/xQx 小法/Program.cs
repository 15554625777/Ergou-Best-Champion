...
namespace VeigarEndboss
{
    class Program
    {
        ...
        private static void SetuptMenu()
        {
            ...
            waveClear.AddItem(new MenuItem("waveNumW", "W 小兵数量").SetValue(new Slider(3, 1, 10)));
            waveClear.AddItem(new MenuItem("waveActive", "启动 清线").SetValue(new KeyBind('V', KeyBindType.Press)));
            menu.AddSubMenu(waveClear);

            // Misc
            Menu misc = new Menu("杂项", "misc");
            misc.AddItem(new MenuItem("miscStackQ", "自动 叠 Q").SetValue(true));
