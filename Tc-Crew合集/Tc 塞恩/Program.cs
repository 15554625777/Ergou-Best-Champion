...
namespace Sion
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.SubMenu("R").AddItem(new MenuItem("AntiCamLock", "避免锁定镜头").SetValue(true));
            Config.SubMenu("R").AddItem(new MenuItem("MoveToMouse", "鼠标控制方向 (利用bug)").SetValue(false));
                //Disabled by default since its not legit Keepo


            Config.AddToMainMenu();

            Game.PrintChat("Tc濉炴仼涓ㄥ姞杞芥垚鍔燂紒姹夊寲by Bbyyyyy锛丵Q缇や辅361630846!");
