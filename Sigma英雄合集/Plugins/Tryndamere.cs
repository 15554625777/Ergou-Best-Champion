...
namespace SigmaSeries.Plugins
{
    public class Tryndamere : PluginBase
    {
        ...
        {
            config.AddItem(new MenuItem("UseQCombo", "使用 Q").SetValue(false));
            config.AddItem(new MenuItem("minQHP", "使用 Q 最低血量").SetValue(new Slider(15, 1, 100)));
            config.AddItem(new MenuItem("smartSep", "- - 智能 W 设置").SetValue(true));
            config.AddItem(new MenuItem("UseWCombo", "使用 W").SetValue(true));
            config.AddItem(new MenuItem("wBack", "使用 W 正对敌人 + 离开平A范围").SetValue(true));
            config.AddItem(new MenuItem("wFront", "使用 W 在平A范围").SetValue(false));
            config.AddItem(new MenuItem("UseECombo", "使用 E 切入 敌方").SetValue(false));
            config.AddItem(new MenuItem("UseRCombo", "使用 R!").SetValue(true));
            config.AddItem(new MenuItem("minRHP", "使用 R 最低血量").SetValue(new Slider(5, 1, 100)));
        }

        public override void HarassMenu(Menu config)
        {
            config.AddItem(new MenuItem("extraPARRAMAMAMAM", "使用混合模式, 禁用技能."));
        }

        public override void FarmMenu(Menu config)
        {
            config.AddItem(new MenuItem("useEWC", "使用 E 清线").SetValue(false));
            config.AddItem(
            ...
            config.AddItem(new MenuItem("minQHPJung", "使用 Q 最低血量").SetValue(new Slider(15, 1, 100)));
        }
