...
namespace SigmaSeries.Plugins
{
    public class Varus : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("minQ", "使用 Q 叠加被动层数").SetValue(new Slider(2, 0, 3)));
            config.AddItem(new MenuItem("UseECombo", "使用 E").SetValue(true));
            config.AddItem(new MenuItem("minE", "使用 E 叠加被动层数").SetValue(new Slider(2, 0, 3)));
            config.AddItem(new MenuItem("UseRCombo", "使用 R").SetValue(false));
            config.AddItem(new MenuItem("forceR", "手动 R").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
        }
        ...
        {
            config.AddItem(new MenuItem("useQFarm", "使用 Q").SetValue(new StringList(new[] { "控线", "清线", "同时", "禁用" }, 3)));
            config.AddItem(new MenuItem("useEFarm", "使用 E").SetValue(new StringList(new[] { "控线", "清线", "同时", "禁用" }, 3)));
            ...
            config.AddItem(new MenuItem("UseEJung", "使用 E").SetValue(true));
        }
