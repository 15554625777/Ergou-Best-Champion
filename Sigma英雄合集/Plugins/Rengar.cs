...
namespace SigmaSeries.Plugins
{
    public class Rengar : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("UseItems", "使用 I物品").SetValue(true));
            config.AddItem(new MenuItem("sep", "-- 智能堆叠残暴值"));
            config.AddItem(new MenuItem("smart5Stack", "使用 残暴值 技能").SetValue(false));
            config.AddItem(new MenuItem("q1", "使用Q|目标在平A范围.").SetValue(false));
            config.AddItem(new MenuItem("w1", "使用W 低于 %血量").SetValue(false));
            config.AddItem(new MenuItem("wSlide", "%血量设置").SetValue(new Slider(15, 0, 100)));
            config.AddItem(new MenuItem("e1", "使用E|目标超出平A范围").SetValue(false));
            config.AddItem(new MenuItem("sep2", "-- 禁用智能堆叠残暴值"));
            config.AddItem(new MenuItem("stackPriority", "5层残暴值优先使用").SetValue(new StringList(new[] { "Q", "W", "E" }, 0)));
            config.AddItem(new MenuItem("tripleQ", "三倍 Q!").SetValue(new KeyBind("M".ToCharArray()[0], KeyBindType.Press)));
        }

        public override void HarassMenu(Menu config)
        {
            config.AddItem(new MenuItem("UseWHarass", "使用 W").SetValue(true));
            config.AddItem(new MenuItem("UseEHarass", "使用 E").SetValue(true));
        }

        public override void FarmMenu(Menu config)
        {
            config.AddItem(new MenuItem("sep22", "-- 控线 设置"));
            config.AddItem(new MenuItem("qf", "使用 Q").SetValue(false));
            config.AddItem(new MenuItem("wf", "使用 W").SetValue(false));
            config.AddItem(new MenuItem("ef", "使用 E").SetValue(false));
            config.AddItem(new MenuItem("sep33", "-- 清线 设置"));
            config.AddItem(new MenuItem("51", "使用5层残暴值").SetValue(false));
            config.AddItem(new MenuItem("waveClearPriority", "5层残暴值优先使用").SetValue(new StringList(new[] { "Q", "W" }, 0)));
            config.AddItem(new MenuItem("ql", "使用 Q").SetValue(false));
            config.AddItem(new MenuItem("wl", "使用 W").SetValue(false));
            config.AddItem(new MenuItem("el", "使用 E").SetValue(false));
            config.AddItem(new MenuItem("sep", "-- 打野 设置"));
            config.AddItem(new MenuItem("JungleActive", "清野!").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
            config.AddItem(new MenuItem("52", "使用5层残暴值").SetValue(false));
            config.AddItem(new MenuItem("jungPriority", "5层残暴值优先使用").SetValue(new StringList(new[] { "Q", "W" }, 0)));
            config.AddItem(new MenuItem("qj", "使用 Q").SetValue(false));
            config.AddItem(new MenuItem("wj", "使用 W").SetValue(false));
            config.AddItem(new MenuItem("ej", "使用 E").SetValue(false));
        }
